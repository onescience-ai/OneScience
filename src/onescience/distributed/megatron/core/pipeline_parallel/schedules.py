...
from onescience.distributed.megatron.core.pipeline_parallel import p2p_communication, pangu_pp_utils
...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config):
    ...
    output_tensors = []
    output_tensor_grads = []
    for i in range(len(output_tensor_grad)):
        if output_tensor_grad[i] is None and config.grad_scale_func is not None:
            output_tensor[i] = config.grad_scale_func(output_tensor[i])
        ...
        if output_tensor[i].requires_grad:
            output_tensors.append(output_tensor[i])
            output_tensor_grads.append(output_tensor_grad[i])
    if output_tensors:
...
def send_forward(output_tensors, tensor_shapes, config, is_last_stage, is_pangu=False):
...
def send_backward(input_tensor_grads, tensor_shapes, config, is_first_stage):
    ...
    # 补齐长度，确保与 tensor_shapes 一致
    while len(input_tensor_grads) < len(tensor_shapes):
...
def send_forward_recv_backward(output_tensors, tensor_shapes, config, is_last_stage, is_pangu=False):
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    pp_config = getattr(args, 'pipeline_tensor_shape_config', None)
    is_pangu = pp_config is not None
    if pp_config is not None:
        recv_tensor_shapes, send_tensor_shapes = pp_config.get_shapes(rank)
    else:
    ...
    for i in range(num_warmup_microbatches):
        ...
        send_forward(
            output_tensor, send_tensor_shapes, config, parallel_state.is_pipeline_last_stage(), is_pangu
        )
            ...
            if is_pangu:
                ...
                if isinstance(output_tensor, list) and len(output_tensor) > 0:
                    ...
                    if isinstance(out, tuple):
                        for t in out:
                            if isinstance(t, torch.Tensor):
                                deallocate_output_tensor(t, config.deallocate_pipeline_outputs)
                    elif isinstance(out, torch.Tensor):
                        deallocate_output_tensor(out, config.deallocate_pipeline_outputs)
            else:
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            ...
            if is_pangu:
                ...
                if isinstance(out, tuple):
                    output_tensor_list = list(out)
                    output_tensor_grad_list = output_tensor_grad if isinstance(output_tensor_grad, list) else [output_tensor_grad]
                    # 补齐长度
                    while len(output_tensor_grad_list) < len(output_tensor_list):
                        output_tensor_grad_list.append(None)
                else:
                    output_tensor_list = [out]
                    output_tensor_grad_list = [output_tensor_grad] if not isinstance(output_tensor_grad, list) else output_tensor_grad

                 # 处理 input_tensor
                if input_tensor == [None] or input_tensor is None:
                    input_tensor_list = [None] * len(output_tensor_list)
                ...
                input_tensor_grad = backward_step(
                    input_tensor_list, output_tensor_list, output_tensor_grad_list, model_type, config
                )
            else:
                input_tensor_grad = backward_step(
                    input_tensor, output_tensor, output_tensor_grad, model_type, config
            ...
            # Send backward and receive forward
            if last_iteration:
                input_tensor = None
                send_backward(
                    input_tensor_grad,
