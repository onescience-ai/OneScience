...
def recv_forward(tensor_shapes, config, is_first_stage):
    ...
    #import traceback
    #traceback.print_stack()
    #print("finish recive ... dat:"+str(input_tensors))
...
def send_forward(output_tensors, tensor_shapes, config, is_last_stage):
    ...
    new_tensors = []
    #print("send output:"+str(output_tensors))
    for item in output_tensors:
        #print(item)
        if isinstance(item,tuple):
            new_tensors.append(item[0])
            new_tensors.append(item[1])
            # new_tensors.append(torch.tensor([item[2],item[3],item[4],item[5]]).half().cuda())
            new_tensors.append( # Modification points. 修改点(fp32)
                torch.tensor(
            ...
            #new_tensors.append(torch.tensor([item[3]]).cuda())
            #new_tensors.append(torch.tensor([item[4]]).cuda())
            #new_tensors.append(torch.tensor([item[5]]).cuda())
    #print(" send :",new_tensors)
    for output_tensor in new_tensors:
        #print("start ...")
        p2p_communication.send_forward(output_tensor, config, is_last_stage)
        #print("end ...")
    #print("finish send... dat:"+str(new_tensors))
...
def send_forward_recv_backward(output_tensors, tensor_shapes, config, is_last_stage):
    ...
    return output_tensor_grads
    '''
    #print("start send_forward_recv_backward... is_last_stage:"+str(is_last_stage))
    new_tensors = []
    #tensor_shapes = []
    for item in output_tensors:
        if isinstance(item,tuple):
    ...
    output_tensor_grads = []
    #print("new_tensors: "+str(new_tensors))
    #print("tensor_shapes: "+str(tensor_shapes))
    for output_tensor, tensor_shape in zip(new_tensors, tensor_shapes):
        if tensor_shape is None:
            output_tensor_grads.append(None)
            continue
        output_tensor_grad = p2p_communication.send_forward_recv_backward(
        ...
        output_tensor_grads.append(output_tensor_grad)
    #print("end send_forward_recv_backward... is_last_stage:"+str(is_last_stage))
    return output_tensor_grads
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    #print("rank:"+str(rank)+"recv shape:",recv_tensor_shapes)
    #print("rank:"+str(rank)+"send shape:",send_tensor_shapes)
    ...
    if not forward_only:
        ...
        output_tensors = []
    forward_data_store = []
    #print("pos 1 rank:"+str(rank)+" microbatches:"+str(num_warmup_microbatches))
    # Run warmup forward passes.
    for i in range(num_warmup_microbatches):
        ...
        #print("pos 1-2 rank:"+str(rank)+" loop idx:"+str(i)+" recv_input:"+ str(input_tensor))
        output_tensor, num_tokens = forward_step(
            forward_step_func,
            data_iterator,
            model,
        ...
        #print(output_tensor[0][0].shape)
        #print(output_tensor[0][1].shape)
        #print("pos 1-2 rank:"+str(rank)+" loop idx:"+str(i)+" output:"+ str(output_tensor))
        send_tensor_shapes = [[1, 131040, dim_size*2], [1, 521280, dim_size], [4]] # Modification points. 修改点(fp32)
        send_forward(
            output_tensor, send_tensor_shapes, config, parallel_state.is_pipeline_last_stage()
        )
        #print("pos 1-2 rank:"+str(rank)+" loop idx:"+str(i)+"send end")
        total_num_tokens += num_tokens
        ...
        if not forward_only:
            ...
            #print("config.deallocate_pipeline_outputs"+str(config.deallocate_pipeline_outputs))
            config.deallocate_pipeline_outputs = False
            deallocate_output_tensor(output_tensor[0][0], config.deallocate_pipeline_outputs)
            deallocate_output_tensor(output_tensor[0][1], config.deallocate_pipeline_outputs)
        #print("pos 1-2 rank:"+str(rank)+"loop idx:"+str(i))
    #print("pos 2 rank:"+str(rank)+" num_microbatches_remaining:"+str(num_microbatches_remaining))
    # Before running 1F1B, need to receive first forward tensor.
    # If all microbatches are run in warmup / cooldown phase, then no need to
    # receive this tensor here.
    recv_tensor_shapes = [[1, 131040, dim_size*2],[1, 521280, dim_size],[4]]
    if num_microbatches_remaining > 0:
        #print("start recive .......")
        input_tensor = recv_forward(
            recv_tensor_shapes, config, parallel_state.is_pipeline_first_stage()
        )
    #print("pos 2 - rank:"+str(rank))
    # Run 1F1B in steady state.
    for i in range(num_microbatches_remaining):
        #print("pos 2-3 start rank:"+str(rank)+"loop idx:"+str(i))
        ...
        total_num_tokens += num_tokens
        #print("pos 2-3 endforwad rank:"+str(rank)+"loop idx:"+str(i))
        if forward_only:
            send_forward(
                output_tensor, send_tensor_shapes, config, parallel_state.is_pipeline_last_stage()
            )
            ...
            #print("pos 2-3 start send rank:"+str(rank)+"loop idx:"+str(i))
            send_tensor_shapes = [[1, 131040, dim_size*2],[1, 521280, dim_size],[4]]
            output_tensor_grad = send_forward_recv_backward(
                output_tensor, send_tensor_shapes, config, parallel_state.is_pipeline_last_stage()
            )
            #print("pos 2-3 end send  rank:"+str(rank)+"loop idx:"+str(i))
            ...
            if num_warmup_microbatches == 0 and last_iteration:
                if config.grad_sync_func is None or rank == 0:
                    enable_grad_sync()
            #print("input_tensor:"+str(input_tensor[0].shape)+str(input_tensor[1].shape))
            #print("output_tensor:"+str(output_tensor))
            #print("output_tensor_grad:"+str(output_tensor_grad))
            #print("input_tensor:"+str(input_tensor))
            if input_tensor==[None]:
                output_tensor = [output_tensor[0][0],output_tensor[0][1]]
                output_tensor_grad = [output_tensor_grad[0],output_tensor_grad[1]] 
                input_tensor_grad = backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config)
            else:
                output_tensor_grad.append(None)
                if len(output_tensor)==1 and isinstance(output_tensor[0],tuple):
                    output_tensor = [output_tensor[0][0],output_tensor[0][1]]
                    output_tensor_grad = [output_tensor_grad[0],output_tensor_grad[1]]
                input_tensor_grad = backward_step(
                    [input_tensor[0],input_tensor[1]], output_tensor, output_tensor_grad, model_type, config
                )

            #input_tensor_grad_1 = backward_step(
            #    input_tensor[1], output_tensor, output_tensor_grad, model_type, config
            #)
            #input_tensor_grad = [input_tensor_grad_0[0],input_tensor_grad_1[1]]
            #print("input_tensor_grad:"+str(input_tensor_grad))
            #print("pos 2-3 end backward rank:"+str(rank)+"loop idx:"+str(i))
            recv_tensor_shapes = [[1, 131040, dim_size*2],[1, 521280, dim_size],[4]]
            if len(input_tensor_grad)==2:
                # input_tensor_grad.append(torch.tensor([0,0,0,0]).half().cuda())
                input_tensor_grad.append( # Modification points. 修改点(fp32)
                    torch.tensor(
                        [0, 0, 0, 0],
                        device="cuda")
                        .to(dtype=config.pipeline_dtype, non_blocking=True)
            ...
            if last_iteration:
                #print("last true")
                #print("input_tensor_grad:"+str(input_tensor_grad))
                if input_tensor_grad!=[None]:
                    input_tensor_grad = [input_tensor_grad[0],input_tensor_grad[1]]
                    recv_tensor_shapes = [[1, 131040, dim_size*2],[1, 521280, dim_size]]
                input_tensor = None
                send_backward(
                    input_tensor_grad,
                    recv_tensor_shapes,
            ...
            #print("input_tensor:"+str(input_tensor))
            #print("recv_tensor_shapes:"+str(recv_tensor_shapes))
            #print("send end")
            #print("pos 2-3 end send backward rank:"+str(rank)+"loop idx:"+str(i))
        #print("pos 2-3 end rank:"+str(rank)+"loop idx:"+str(i))
    #print("pos 3 rank:"+str(rank))
    send_tensor_shapes = [[1, 131040, dim_size*2],[1, 521280, dim_size]]
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):
            ...
            # bubble.
            #print("pos 3-4 end rank:"+str(rank))
            ...
            #print("pos 3-4 end backward rank:"+str(rank))
            send_backward(
                input_tensor_grad,
                recv_tensor_shapes,
                config,
    ...
    if (
        ...
        create_cudagraphs()
    #print("down pipeline!! pos 5 rank:"+str(rank))
    return forward_data_store
