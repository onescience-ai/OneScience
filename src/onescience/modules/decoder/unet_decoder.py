...
class UNetDecoder1D(nn.Module):
    ...
    接收来自编码器的特征列表。它通过 Up1D 模块逐层上采样深层瓶颈特征，
    并与编码器中对应的浅层特征进行跳跃连接 (Skip Connection) 融合。

    Args:
        base_channels (int, optional): 与编码器匹配的初始特征通道数。默认值: 16。
        num_stages (int, optional): 上采样的层数。需与编码器一致。默认值: 2。
        bilinear (bool, optional): 是否使用线性插值进行上采样。默认值: True。
        normtype (str, optional): 归一化类型 ('bn' 或 'in')。默认值: 'bn'。
        kernel_size (int, optional): 卷积核大小，必须为奇数。默认值: 3。

    形状:
        输入 features: list of Tensors (由 UNetEncoder1D 输出的特征列表)。
        输出: (B, base_channels, L)，恢复到输入分辨率的深层特征。

    Example:
        >>> decoder = UNetDecoder1D(base_channels=64, num_stages=4, kernel_size=5)
        >>> # features 是 encoder 的输出
        >>> out = decoder(features)
        >>> print(out.shape)
        torch.Size([8, 64, 128])
    """
    def __init__(self, base_channels=16, num_stages=2, bilinear=True, normtype="bn", kernel_size=3):
        super().__init__()
...
class UNetDecoder2D(nn.Module):
    ...
    Example:
        >>> decoder = UNetDecoder2D(base_channels=32, num_stages=3, kernel_size=3)
        >>> # 假设经过 encoder 输出的 features 列表
        >>> out = decoder(features)
        >>> print(out.shape)
...
class UNetDecoder3D(nn.Module):
    ...
    Example:
        >>> decoder = UNetDecoder3D(base_channels=16, num_stages=2)
        >>> # features 是 encoder3D 的输出
        >>> out = decoder(features)
        >>> print(out.shape)
