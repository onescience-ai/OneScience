...
class EarthTransformer2DBlock(nn.Module):
    """
        二维 Earth Transformer block。

        该模块是面向二维 patch 网格的局部窗口 Transformer block，
        内部结构为：

        - `LayerNorm`
        - 可选循环移位
        - `OneAttention(style="EarthAttention2D")`
        - 残差连接
        - `Mlp`
        - 残差连接

        其中窗口注意力在二维网格 `(Height, Width)` 上执行，并带有地球位置偏置。
        当输入分辨率不能整除窗口大小时，模块会先 padding，再在输出端 crop 回原尺寸。

        Args:
            dim (int):
                输入与输出 token 特征维度。
            input_resolution (tuple[int, int]):
                输入二维网格尺寸 `(Height, Width)`。
            num_heads (int):
                多头注意力头数。
            window_size (tuple[int, int] | None):
                局部窗口大小 `(WindowHeight, WindowWidth)`。
            shift_size (tuple[int, int] | None):
                循环移位大小 `(ShiftHeight, ShiftWidth)`。
                当为 `(0, 0)` 时，不启用 shifted window。
    ...
    def __init__(
        ...
        ShiftHeight, ShiftWidth = self.shift_size
        self.use_roll = bool(ShiftHeight or ShiftWidth)
    ...
    def forward(self, x: torch.Tensor):
        ...
        _, PaddedHeight, PaddedWidth, _ = x.shape

        ShiftHeight, ShiftWidth = self.shift_size
        if self.use_roll:
            shifted_x = torch.roll(
                x,
                shifts=(-ShiftHeight, -ShiftWidth),
                dims=(1, 2),
            )
        ...
        WindowHeight, WindowWidth = self.window_size
        ...
        if self.use_roll:
            ...
            x = torch.roll(shifted_x, shifts=(ShiftHeight, ShiftWidth), dims=(1, 2))
        else:
            shifted_x = window_reverse(
                attn_windows,
                self.window_size,
