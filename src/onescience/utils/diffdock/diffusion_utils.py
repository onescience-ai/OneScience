...
from onescience.utils.diffdock.geometry import (
    axis_angle_to_matrix,
    rigid_transform_Kabsch_3D_torch,
    rigid_transform_Kabsch_3D_torch_batch,
)
from onescience.utils.diffdock.torsion import (
...
def modify_conformer(data, tr_update, rot_update, torsion_updates, pivot=None):
    ligand_edge_store = get_ligand_edge_store(data)
    ...
    if torsion_updates is not None:
        flexible_new_pos = modify_conformer_torsion_angles(
            rigid_new_pos,
            ligand_edge_store.edge_index.T[data["ligand"].edge_mask],
...
def modify_conformer_batch(orig_pos, data, tr_update, rot_update, torsion_updates, mask_rotate):
    ligand_edge_store = get_ligand_edge_store(data)
    B = data.num_graphs
    N = data["ligand"].num_nodes // B
    M = ligand_edge_store.edge_index.shape[1] // B

    pos = orig_pos.reshape(B, N, 3) + 0
    edge_index = ligand_edge_store.edge_index[:, :M]
    edge_mask = data["ligand"].edge_mask[:M]
