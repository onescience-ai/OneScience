...
from onescience.utils.diffdock import get_ligand_edge_store
from onescience.utils.diffdock.diffusion_utils import (
    modify_conformer_batch,
    set_time,
)
from onescience.utils.diffdock.logging_utils import get_logger
from onescience.utils.diffdock.torsion import modify_conformer_torsion_angles
from onescience.utils.diffdock.utils import crop_beyond


def _resolve_mask_rotate(mask_rotate):
    return mask_rotate if isinstance(mask_rotate, np.ndarray) else mask_rotate[0]
...
def randomize_position(
    ...
    if pocket_knowledge:
        complex_graph = data_list[0]
        d = torch.cdist(
            complex_graph["receptor"].pos,
            torch.from_numpy(complex_graph["ligand"].orig_pos[0]).float() - complex_graph.original_center,
        )
    ...
    if not no_torsion:
        for complex_graph in data_list:
            ligand_edge_store = get_ligand_edge_store(complex_graph)
            ...
            complex_graph["ligand"].pos = modify_conformer_torsion_angles(
                ...
                ligand_edge_store.edge_index.T[complex_graph["ligand"].edge_mask],
                _resolve_mask_rotate(complex_graph["ligand"].mask_rotate),
                torsion_updates,
            )
    ...
    for complex_graph in data_list:
        ...
        complex_graph["ligand"].pos = (
            (complex_graph["ligand"].pos - molecule_center) @ random_rotation.T + center_pocket
        )
...
def sampling(
    ...
    mask_rotate = torch.from_numpy(_resolve_mask_rotate(data_list[0]["ligand"].mask_rotate)).to(device)
    ...
    with torch.no_grad():
        for batch_id, complex_graph_batch in enumerate(loader):
            ...
            for i in range(b):
                data_list[batch_id * batch_size + i]["ligand"].pos = complex_graph_batch["ligand"].pos[
                    i * n : n * (i + 1)
                ]
...
def compute_affinity(
    ...
    with torch.no_grad():
        if affinity_model is not None:
            ...
            ligand_edge_store = get_ligand_edge_store(complex_graph)
            ligand_edge_store.edge_index = torch.cat(
                [N * i + ligand_edge_store.edge_index for i in range(parallel)],
                dim=1,
            )
            ligand_edge_store.edge_attr = ligand_edge_store.edge_attr.repeat(parallel, 1)
