...
from onescience.datapipes.diffdock.parse_chi import aa_idx2aa_short, get_onehot_sequence
...
def _require_esm():
    if esm is None or FastaBatchedDataset is None or pretrained is None:
        raise ImportError(
            "esm is required for ESM embedding generation or ESMFold structure generation."
        )
        ...
        if lm_embeddings and (precomputed_lm_embeddings is None or precomputed_lm_embeddings[0] is None):
            ...
            for i in range(len(protein_sequences)):
                ...
                self.lm_embeddings.append(
                    [lm_embeddings[f"{complex_names[i]}_chain_{j}"] for j in range(len(s))]
                )
