...
from onescience.utils.diffdock import get_ligand_edge_store, so3, torus
...
def _debug_raise_enabled():
    return os.environ.get("DIFFDOCK_DEBUG_RAISE", "").lower() in {"1", "true", "yes", "on"}
...
def loss_function(
    ...
    if not no_torsion:
        ...
        if apply_mean:
            ...
            ligand_edge_store = None if device.type == "cuda" else get_ligand_edge_store(data)
            index = (
                torch.cat([torch.ones(d["ligand"].edge_mask.sum()) * i for i, d in enumerate(data)]).long()
                if device.type == "cuda"
                else data["ligand"].batch[ligand_edge_store.edge_index[0][data["ligand"].edge_mask]]
            )
    ...
    if backbone_weight > 0:
        ...
        if apply_mean:
            backbone_loss = backbone_loss.mean() * torch.ones(1, dtype=torch.float)
            backbone_base_loss = backbone_base_loss.mean() * torch.ones(1, dtype=torch.float)
        else:
            index = (
                torch.cat([torch.ones((d["receptor"].pos.shape[0])) * i for i, d in enumerate(data)], dim=0).long()
                if device.type == "cuda"
        ...
        if apply_mean:
            ...
            backbone_loss = torch.zeros(len(rot_loss), dtype=torch.float)
            backbone_base_loss = torch.zeros(len(rot_loss), dtype=torch.float)
    ...
    if sidechain_weight > 0:
        ...
        if apply_mean:
            sidechain_loss = sidechain_loss.mean().cpu() * torch.ones(1, dtype=torch.float)
            sidechain_base_loss = sidechain_base_loss.mean().cpu() * torch.ones(1, dtype=torch.float)
        else:
            index = (
                torch.cat([torch.ones((d["receptor"].pos.shape[0])) * i for i, d in enumerate(data)], dim=0).long()
                if device.type == "cuda"
        ...
        if apply_mean:
            ...
            sidechain_loss = torch.zeros(len(rot_loss), dtype=torch.float)
            sidechain_base_loss = torch.zeros(len(rot_loss), dtype=torch.float)
...
class AverageMeter:
    ...
    def summary(self):
        if self.intervals == 1:
            return {k: v.item() / self.count for k, v in self.acc.items()}
        out = {}
...
def train_epoch(model, loader, optimizer, device, t_to_sigma, loss_fn, ema_weights):
    ...
    if meter.count == 0:
        if last_error is not None:
            raise RuntimeError(
                "No valid training batches completed. The last RuntimeError traceback is shown above."
            ) from last_error
        raise RuntimeError("No valid training batches completed.")
