...
# Backward-compatible re-export; implementation lives in monitoring/uma.
from onescience.monitoring.uma.runtime_logging import (
...
def setup_imports(config: dict | None = None) -> None:
    ...
    try:
        ...
        importlib.import_module("onescience.monitoring.uma.logger")
...
def _resolve_scale_factor_submodule(model: nn.Module, name: str):
    from onescience.utils.uma.scaling.scale_factor import ScaleFactor
...
def get_commit_hash() -> str:
    ...
    try:
        import importlib
        uma_module = importlib.import_module("onescience.models.UMA")
        core_hash = get_commit_hash_for_repo(uma_module.__path__[0])
    except Exception:
        core_hash = None

    try:
        import importlib
        experimental_module = importlib.import_module("onescience.models.experimental")
        experimental_hash = get_commit_hash_for_repo(experimental_module.__path__[0])
    except Exception:
        experimental_hash = None

    return f"UMA:{core_hash or 'NA'},experimental:{experimental_hash or 'NA'}"
