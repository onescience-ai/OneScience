...
def image_batching(
    ...
    padded_shape_x = (
        (patch_shape_x - overlap_pix -
         boundary_pix) * (patch_num_x - 1)
        + patch_shape_x
        + boundary_pix
    )
    padded_shape_y = (
        (patch_shape_y - overlap_pix -
         boundary_pix) * (patch_num_y - 1)
        + patch_shape_y
        + boundary_pix
    )
    ...
    if input_interp is not None:
        ...
        output = torch.zeros(
            patch_num *
            batch_size, input.shape[1], patch_shape_x, patch_shape_y
        ).cuda()
...
def image_fuse(
    ...
    pad_x_right = padded_shape_x - img_shape_x - boundary_pix
    pad_y_right = padded_shape_y - img_shape_y - boundary_pix
    # residual pixels in the last patch
    residual_x = patch_shape_x - pad_x_right
    # residual pixels in the last patch
    residual_y = patch_shape_y - pad_y_right
    output = torch.zeros(
        batch_size, input.shape[1], img_shape_x, img_shape_y).cuda()
    one_map = torch.ones(
        1, 1, input.shape[2], input.shape[3]).cuda()
...
def stochastic_sampler(
    ...
    randn_like: Callable[[Tensor],
                         Tensor] = torch.randn_like,
    img_shape: int = 448,
    patch_shape: int = 448,
    overlap_pix: int = 4,
    boundary_pix: int = 2,
    ...
    if mean_hr is not None:
        x_lr = torch.cat(
            (mean_hr.expand(x_lr.shape[0], -1, -1, -1), x_lr), dim=1)
    global_index = None
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        gamma = (
            min(S_churn / num_steps, np.sqrt(2) -
        ...
        if i < num_steps - 1:
            if patch_shape != img_shape:
                ...
                x_next_batch = x_next
            denoised = net(x_next_batch, x_lr, t_next, class_labels).to(
                torch.float64)
            if patch_shape != img_shape:
                denoised = image_fuse(
                    denoised,
