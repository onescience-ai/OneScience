...
def calculate_mesh_cell_area(unstructured_grid_data):
    # Read VTK file
    poly_data, _ = unstructured_grid_data_to_poly_data(
...
def calculate_cell_velocity_gradient(unstructured_grid_data, velocity):
    ...
    velocity_data.SetNumberOfComponents(
        3)  # Assuming 3D velocity field
    velocity_data.SetNumberOfTuples(
        unstructured_grid_data.GetNumberOfPoints())
    # Replace "Velocity" with the desired array name
    ...
    while cells.GetNextCell(cell):
        ...
        if cell.GetNumberOfIds() == 4:
            ...
            u1 = np.array(
                poly_data.GetPointData().GetArray(
                    "Velocity").GetTuple(cell.GetId(0))
            )
            u2 = np.array(
                poly_data.GetPointData().GetArray(
                    "Velocity").GetTuple(cell.GetId(1))
            )
            u3 = np.array(
                poly_data.GetPointData().GetArray(
                    "Velocity").GetTuple(cell.GetId(2))
            )
            u4 = np.array(
                poly_data.GetPointData().GetArray(
                    "Velocity").GetTuple(cell.GetId(3))
            )

            # Calculate the gradients using finite differences
            du_dx = (u2 - u1 + u3 - u4) / \
                (np.linalg.norm(p2 - p1 + p3 - p4) + 1e-8)
            du_dy = (u3 - u1 + u4 - u2) / \
                (np.linalg.norm(p3 - p1 + p4 - p2) + 1e-8)
            du_dz = (u4 - u1 + u2 - u3) / \
                (np.linalg.norm(p4 - p1 + p2 - p3) + 1e-8)
...
def calculate_drag_force(
    ...
    pressure_force_component = -np.dot(
        pressure_array.flatten() *
        cell_areas.flatten(), surface_normals.flatten()
    )
    ...
    drag_force = np.sum(
        pressure_force_component + wall_shear_stress_component)
...
def cal_coefficient(data_dir, press_surf=None, velo_surf=None):
    file_name_press = os.path.join(
        data_dir, "quadpress_smpl.vtk")
    file_name_velo = os.path.join(
        data_dir, "hexvelo_smpl.vtk")
    ...
    # gradient u
    grad_u = calculate_cell_velocity_gradient(
        unstructured_grid_data_press, velo_surf)
    # press
    if press_surf is None:
        ...
        press_data.SetNumberOfComponents(
            1)  # Assuming 3D velocity field
        press_data.SetNumberOfTuples(
            unstructured_grid_data_press.GetNumberOfPoints())
        # Replace "my_press" with the desired array name
    ...
    drag_force = calculate_drag_force(
        cell_areas, normal_surf[:, -
                                1], press_surf, grad_u[:, -1], np.array(1.8e-5)
    )
    nu = 72 / 3.6
    air_density = 0.3
