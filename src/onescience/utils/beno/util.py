...
def plot_matrices(
    ...
    if scale_limit == "auto":
        ...
        if is_balanced:
            scale_min, scale_max = -max(abs(scale_min), abs(scale_max)), max(
...
class Early_Stopping(object):
    ...
    def __repr__(self):
        return "Early_Stopping(patience={}, epsilon={}, mode={}, wait={})".format(
            self.patience, self.epsilon, self.mode, self.wait
        )
...
def get_tril_block(size, block_size, diagonal=0):
    ...
    mesh1, mesh2 = torch.meshgrid(
        torch.arange(1, 1 + size), torch.arange(1, 1 + size), indexing="ij"
    )
    ...
    **kwargs,
    ...
    if "+" in loss_type:
        ...
        for loss_component in loss_type.split("+"):
            if precision_floor is not None and not (
                "mselog" in loss_component
...
def check_same_model_dict(model_dict1, model_dict2):
    ...
    assert set(model_dict1.keys()) == set(
        model_dict2.keys()
    ), "model_dict1 and model_dict2 has different keys!"
    for key, item1 in model_dict1.items():
        item2 = model_dict2[key]
        if not isinstance(item1, dict):
            assert (
                item1 == item2
            ), "key '{}' has different values of '{}' and '{}'.".format(
                key, item1, item2
            )
    return True
...
def get_filename_short(
    ...
    for key in args_shown:
        if key in args_dict:
            ...
            raise Exception(
                "'{}' not in the short_str_dict. Need to add its short name into it.".format(
                    key
                )
            )
...
class Batch(object):
    ...
    def collate(self):
        ...
        if (
            ...
            or torch.__version__.startswith("1.10")
            or torch.__version__.startswith("1.11")
            or torch.__version__.startswith("1.12")
        ):
...
def to_device_recur(iterable, device, is_detach=False):
    if isinstance(iterable, list):
        ...
        return tuple(
            to_device_recur(item, device, is_detach=is_detach) for item in iterable
        )
...
def slice_divmod(slice_item, denom):
    ...
    assert (
        len(sim_id_set) == 1
    ), "The number of returned sim_id must be 1. Here the sim_id are: {}".format(
        sim_id_set
    )
    sim_id = sim_id_set[0]
    ...
    for k in range(length):
        ...
        for tuple_item in tuples:
            ...
            if tuple_item[k] is None or is_None:
                assert (
                    tuple_item[k] is None
                ), "Either all {}th elements are None or are not None".format(k)
                is_None = True
            else:
...
def forward_Runge_Kutta(model, x, mode="RK4"):
    ...
    if mode.startswith("RK"):
        ...
        assert tuple_shape_length_equal(
            k1, x
...
def build_optimizer(args, params):
    ...
    if args.lr_scheduler_type == "rop":
        ...
        scheduler = lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=args.lr_scheduler_T0, T_mult=args.lr_scheduler_T_mult
        )
        args.epochs = epochs_new
        print(
            "Reset epochs to {} to locate on the cumulative of geometric series.".format(
                args.epochs
            )
        )
...
def get_cosine_decay(value_start, value_end, n_iter):
    array = (
        value_end
        + (1 + torch.cos(torch.arange(n_iter + 1) / n_iter * torch.pi))
...
def gather_broadcast(tensor, dim, index):
    ...
    assert (
        len(index.shape) == dim
        and tensor.shape[:dim] == index.shape
    ...
    for string_item in cmd.split(" "):
        if string_item.startswith("--"):
            ...
            try:
                ...
                if (
                    isinstance(value_candidate, Number)
                    or isinstance(value_candidate, bool)
                ) and key not in str_keys:
...
def try_call(
    fun, args=None, kwargs=None, time_interval=5, max_n_trials=20, max_exp_time=None
):
    ...
    for i, time_interval in enumerate(time_interval_list):
        ...
        if is_succeed:
            ...
            print(
                "Fail to execute function {} for the {}th time, with error: {}".format(
                    fun, i + 1, error
                )
            )
    ...
    if not is_succeed:
        raise Exception(
            "Fail to execute function {} for the {}th time, same as the max_n_trials of {}. Check error!".format(
                fun, i + 1, max_n_trials
            )
        )
    return output
...
def get_soft_IoU(mask1, mask2, dim, epsilon=1):
    ...
    if isinstance(mask1, np.ndarray):
        soft_IoU = (mask1 * mask2).sum(dim) / (mask1 + mask2 - mask1 * mask2).sum(
            dim
        ).clip(epsilon, None)
    else:
        soft_IoU = (mask1 * mask2).sum(dim) / (mask1 + mask2 - mask1 * mask2).sum(
...
def draw_nx_graph(g):
    ...
    nx.draw_networkx_edge_labels(
        ...
        edge_labels=(
            {
                (edge_src, edge_dst): item["type"]
                for edge_src, edge_dst, item in g.edges(data=True)
            }
            if "type" in list(g.edges(data=True))[0][2]
            else None
        ),
        font_color="red",
    )
...
def func_recursive(item, func, atomic_class=None, *args, **kwargs):
    ...
    if atomic_class is not None and isinstance(item, atomic_class):
        ...
        return {
            key: func_recursive(
                value, func=func, atomic_class=atomic_class, *args, **kwargs
            )
            for key, value in item.items()
        }
    elif isinstance(item, tuple):
            ...
            if edge1 != edge2:
                ...
                if len(common_nodes) > 0:
                    ...
                    line_graph.append(
                        (
                            (
                                f"{edge1[0][0]},{edge1[0][1]}",
                                f"{edge2[0][0]},{edge2[0][1]}",
                            ),
                            dict(graph)[common_node],
                        )
                    )
    return line_graph
...
def get_all_graphs(graph):
    ...
    nodes = [
        ele
        for ele in graph
        if not (isinstance(ele[0], tuple) or isinstance(ele[0], list))
    ]
    edges = [
...
def get_matching_mapping(graph, subgraph_cand):
    ...
    for match in DiGM.subgraph_isomorphisms_iter():
        ...
        if is_valid:
            for edge in subgraph_linegraph.edges:
                ...
                if (
                    subgraph_edge_type != ""
                    and subgraph_edge_type not in graph_edge_type
                ):
                    is_valid = False
                    break
        if is_valid:
    ...
    for node_reverse_mapping in node_reverse_mappings:
        ...
        for subgraph_node, graph_node in node_reverse_mapping.items():
            if (
                subgraph_dict[subgraph_node] != ""
                and subgraph_dict[subgraph_node] not in graph_dict[graph_node]
            ):
                is_valid = False
                break
        if is_valid:
...
def filter_sub_linegraph(alpha_item, task_linegraph_item):
    ...
    linegraph = [
        ...
        if not isinstance(ele[0], tuple) and ele[0] in nodes_chosen
    ]
...
def get_unique_keys_df(df, types="all", exclude=None, exclude_str=None):
    ...
    if types == "all":
        ...
        keys_str = [
            key
            for key, value in zip(df.keys(), df.iloc[0].values)
...
def tsne_torch(embeddings, num_components, verbose=True):
    ...
    embeddings = TSNE(
        n_components=num_components, perplexity=30, n_iter=1000, verbose=verbose
...
def leaky_clamp(x, min, max, slope=0.01):
    ...
    return (
        (nn.LeakyReLU(negative_slope=negative_slope)(x - min) + min)
        + (max - nn.LeakyReLU(negative_slope=negative_slope)(max - x))
    ) / 2
...
class MLP(nn.Module):
    def __init__(
        ...
        if not isinstance(n_neurons, Number):
            assert n_layers == len(
                n_neurons
            ), "If n_neurons is not a number, then its length must be equal to n_layers={}".format(
                n_layers
            )
        ...
        if act_name != "siren":
            ...
            for i in range(1, self.n_layers + 1):
                out_neurons = (
                    self.n_neurons
                    if isinstance(self.n_neurons, Number)
                    else self.n_neurons[i - 1]
                )
                        ...
                        if self.normalization_type != "None":
                            setattr(
                                ...
                                get_normalization(
                                    ...
                                    last_out_neurons,
                                    n_groups=self.normalization_n_groups,
                                ),
                            )
                        setattr(
            ...
            from siren_pytorch import Sine, SirenNet
...
def kernel_condition(
    input, pos_dims, kernel_size, padding_mode, channel_mean_mode="all-mean"
):
    """
        ...
        for kk, mode_str in enumerate(mode_str_list):
            if mode_str == "m":  # "mean"
                weight_ele = (
                    ...
                    / kernel_size**pos_dims
                ).expand(in_channels, in_channels, *kernel_size_tuple)[kk]
            elif mode_str == "z":  # "zero"
                weight_ele = torch.zeros(
                    in_channels, *kernel_size_tuple, device=device, dtype=dtype
                )
...
def filter_dictionary(d, key_func, value_func, return_type="value", return_mode="gen"):
    ...
    if return_mode == "gen":
        for key, value in d.items():
            if (key_func is None is None or key_func(key)) and (
                value_func is None or value_func(value)
            ):
