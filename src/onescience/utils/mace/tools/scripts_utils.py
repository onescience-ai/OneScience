...
from onescience.modules.func_utils.mace_func_utils import compute_avg_num_neighbors
...
#from onescience.utils.mace.data import KeySpecification
from onescience.datapipes.materials.pyg_stack.core.utils import KeySpecification
from onescience.utils.mace.tools.train import SWAContainer
...
def convert_from_json_format(dict_input):
    ...
    if (
        ...
        dict_output["interaction_cls"] = RealAgnosticResidualInteractionBlock
    if (
        dict_input["interaction_cls"]
        == "<class 'mace.modules.blocks.RealAgnosticInteractionBlock'>"
    ):
        dict_output["interaction_cls"] = RealAgnosticInteractionBlock
    if (
        dict_input["interaction_cls_first"]
        == "<class 'mace.modules.blocks.RealAgnosticResidualInteractionBlock'>"
    ):
        dict_output["interaction_cls_first"] = RealAgnosticResidualInteractionBlock
    if (
        dict_input["interaction_cls_first"]
        == "<class 'mace.modules.blocks.RealAgnosticInteractionBlock'>"
    ):
        dict_output["interaction_cls_first"] = RealAgnosticInteractionBlock
...
def load_from_json(f: str, map_location: str = "cpu") -> torch.nn.Module:
    ...
    model_load_yaml = ScaleShiftMACE(
...
def get_loss_fn(
    ...
    if args.loss == "weighted":
        ...
        loss_fn = WeightedForcesLoss(forces_weight=args.forces_weight)
...
def get_swa(
    ...
    if args.loss == "virials":
        loss_fn_energy = WeightedEnergyForcesVirialsLoss(
        ...
        loss_fn_energy = WeightedEnergyForcesStressLoss(
            energy_weight=args.swa_energy_weight,
            forces_weight=args.swa_forces_weight,
        ...
        loss_fn_energy = WeightedEnergyForcesDipoleLoss(
        ...
        loss_fn_energy = WeightedEnergyForcesLoss(
            energy_weight=args.swa_energy_weight,
            forces_weight=args.swa_forces_weight,
        )
        logging.info(
