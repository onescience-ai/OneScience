...
def CFD_1d_loss(pred, a, u, dx, dt):
    ...
    f_loss = (
        ...
        + loss_fn(eq2, torch.zeros_like(eq2))
...
def diff_react_2d_loss(pred, a, u, dx, dt):  #
    ...
    u1 = pred[..., 0]
    u2 = pred[..., 1]
    ic_loss = loss_fn(u1[:, :, :, 0], u[:, :, :, 0, 0]) + loss_fn(
        u2[:, :, :, 0], u[:, :, :, 0, 1]
    )
...
def swe_2d_loss(pred, a, u, dx, dt):
    ...
    eq1 = (
        dh_t[:, 1:-1, 1:-1]
        + dh_x[:, :, 1:-1, 1:-1] * u1[:, 1:-1, 1:-1, 1:-1]
        + h[:, 1:-1, 1:-1, 1:-1] * du1_x[:, :, 1:-1, 1:-1]
        + dh_y[:, 1:-1, :, 1:-1] * u2[:, 1:-1, 1:-1, 1:-1]
        + h[:, 1:-1, 1:-1, 1:-1] * du2_y[:, 1:-1, :, 1:-1]
    )
    eq2 = (
        du1_t[:, 1:-1, 1:-1]
...
def CFD_2d_loss(pred, a, u, dx, dt):
    ...
    ic_loss = (
        loss_fn(h[..., 0, :], u[..., 0, 0:1])
        + loss_fn(u1[..., 0, :], u[..., 0, 1:2])
        + loss_fn(u2[..., 0, :], u[..., 0, 2:3])
        + loss_fn(p[..., 0, :], u[..., 0, 3:4])
    )
    ...
    f_loss = (
        ...
        + loss_fn(eq2, torch.zeros_like(eq2))
        + loss_fn(eq3, torch.zeros_like(eq3))
        + loss_fn(eq4, torch.zeros_like(eq4))
    )
    return ic_loss, f_loss
