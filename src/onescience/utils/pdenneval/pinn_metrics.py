...
def metric_func(pred, target, if_mean=True, Lx=1.0, Ly=1.0, Lz=1.0, iLow=4, iHigh=12):
    ...
    nrm = torch.sqrt(torch.mean(
        target.view([nb, nc, -1, nt]) ** 2, dim=2))
    ...
    err_Max = torch.max(
        torch.max(
            torch.abs(pred.view(
                [nb, nc, -1, nt]) - target.view([nb, nc, -1, nt])),
            dim=2,
        )[0],
        dim=0,
    )[0]
    ...
    if len(idxs) == 5:  # 2D
        ...
        err_F = torch.zeros(
            [nb, nc, min(nx // 2, ny // 2), nt]).to(device)
        for i in range(nx // 2):
            for j in range(ny // 2):
        ...
        _err_F = torch.sqrt(torch.mean(
            err_F, axis=0)) / (nx * ny * nz) * Lx * Ly * Lz
...
def metrics(
        ...
        with torch.no_grad():
            ...
            for xx, yy in val_loader:
                ...
                for t in range(initial_step, yy.shape[-2]):
                    ...
                    temp_shape = [0]
                    temp_shape.extend(
                        [i for i in range(2, len(inp.shape))])
                    temp_shape.append(1)
                    im = model(inp).permute(
                        temp_shape).unsqueeze(-2)
                ...
                _err_RMSE, _err_nRMSE, _err_CSV, _err_Max, _err_BD, _err_F = (
                    metric_func(
                        pred, yy, if_mean=True, Lx=Lx, Ly=Ly, Lz=Lz)
                )
                ...
                if itot == 0:
                    ...
                    pred_plot = pred[:1]
                    target_plot = yy[:1]
                    val_l2_time = torch.zeros(
                        yy.shape[-2]).to(device)
                else:
                    err_RMSE += _err_RMSE
    ...
    print(
        "RMSE of conserved variables: {0:.5f}".format(err_CSV))
    print(
        "Maximum value of rms error: {0:.5f}".format(err_Max))
    ...
    if plot:
        ...
        if dim == 1:
            ...
            h = ax.imshow(
                pred_plot[..., channel_plot].squeeze(
                ).detach().cpu(),
            ...
            fig, ax = plt.subplots(figsize=(6.5, 6))
            h = ax.imshow(
                target_plot[..., channel_plot].squeeze(
                ).detach().cpu(),
                extent=[t_min, t_max, x_min, x_max],
                origin="lower",
                aspect="auto",
            ...
            h = ax.imshow(
                pred_plot[..., -1,
                          channel_plot].squeeze().t().detach().cpu(),
...
class LpLoss(object):
    ...
    def __call__(self, x, y, eps=1e-20):
        ...
        _diff = x.view(num_examples, -1) - \
            y.view(num_examples, -1)
        _diff = torch.norm(_diff, self.p, 1)
        _norm = eps + \
            torch.norm(y.view(num_examples, -1), self.p, 1)
...
class FftLpLoss(object):
    ...
    def __call__(self, x, y, flow=None, fhigh=None, eps=1e-20):
        ...
        _diff = torch.norm(_diff.reshape(
            num_examples, -1), self.p, 1)
        _norm = eps + \
            torch.norm(yf.reshape(
                num_examples, -1), self.p, 1)
...
def inverse_metrics(u0, x, pred_u0, y):
    ...
    # initial condition
    mseloss_u0 = mseloss_fn(
        u0.view(1, -1), x.view(1, -1)).item()
    l2loss_u0 = l2loss_fn(
        u0.view(1, -1), x.view(1, -1)).item()
    l3loss_u0 = l3loss_fn(
        u0.view(1, -1), x.view(1, -1)).item()
    ...
    fftmseloss_mid_u0 = fftmseloss_fn(
        u0, x, fmid, 2 * fmid).item()
    ...
    l2loss_pred_u0 = l2loss_fn(
        pred_u0.reshape(1, -1), y.reshape(1, -1)).item()
    l3loss_pred_u0 = l3loss_fn(
        pred_u0.reshape(1, -1), y.reshape(1, -1)).item()
    ...
    fftmseloss_low_pred_u0 = fftmseloss_fn(
        pred_u0, y, 0, fmid).item()
    fftmseloss_mid_pred_u0 = fftmseloss_fn(
        pred_u0, y, fmid, 2 * fmid).item()
    fftmseloss_hi_pred_u0 = fftmseloss_fn(
        pred_u0, y, 2 * fmid).item()

    fftl2loss_pred_u0 = fftl2loss_fn(pred_u0, y).item()
    fftl2loss_low_pred_u0 = fftl2loss_fn(
        pred_u0, y, 0, fmid).item()
    fftl2loss_mid_pred_u0 = fftl2loss_fn(
        pred_u0, y, fmid, 2 * fmid).item()
    fftl2loss_hi_pred_u0 = fftl2loss_fn(
        pred_u0, y, 2 * fmid).item()

    fftl3loss_pred_u0 = fftl3loss_fn(pred_u0, y).item()
    fftl3loss_low_pred_u0 = fftl3loss_fn(
        pred_u0, y, 0, fmid).item()
    fftl3loss_mid_pred_u0 = fftl3loss_fn(
        pred_u0, y, fmid, 2 * fmid).item()
    fftl3loss_hi_pred_u0 = fftl3loss_fn(
        pred_u0, y, 2 * fmid).item()
...
def bMSE(pred, target):
    ...
    if spatial_dim == 1:
        bd_square_error = (
            pred[:, :, 0, :] - target[:, :, 0, :]) ** 2
        # (bs, v, t)
        bd_square_error += (pred[:, :, -1, :] -
                            target[:, :, -1, :]) ** 2
        bd_mean_square_error = bd_square_error / 2.0
    elif spatial_dim == 2:
        bd_x_square_error = (
            pred[:, :, 0, :, :] - target[:, :, 0, :, :]) ** 2
        bd_x_square_error += (pred[:, :, -1,
                              :, :] - target[:, :, -1, :, :]) ** 2
        bd_x_mean_square_error = torch.mean(
            bd_x_square_error / 2.0, dim=2
        )  # (bs, v, t)
        bd_y_square_error = (
            pred[:, :, :, 0, :] - target[:, :, :, 0, :]) ** 2
        bd_y_square_error += (pred[:, :, :, -1, :] -
                              target[:, :, :, -1, :]) ** 2
        bd_y_mean_square_error = torch.mean(
            bd_y_square_error / 2.0, dim=2
        )  # (bs, v, t)
        bd_mean_square_error = (
            bd_x_mean_square_error + bd_y_mean_square_error) / 2.0
    else:  # spatial_dim == 3
        bd_x_square_error = (
            pred[:, :, 0, :, :, :] - target[:, :, 0, :, :, :]) ** 2
        bd_x_square_error += (pred[:, :, -1, :,
                              :, :] - target[:, :, -1, :, :, :]) ** 2
        bd_x_mean_square_error = torch.mean(
            bd_x_square_error.view([nb, nc, -1, nt]) / 2.0, dim=2
        )  # (bs, v, t)
        bd_y_square_error = (
            pred[:, :, :, 0, :, :] - target[:, :, :, 0, :, :]) ** 2
        bd_y_square_error += (pred[:, :, :, -1,
                              :, :] - target[:, :, :, -1, :, :]) ** 2
        bd_y_mean_square_error = torch.mean(
            bd_y_square_error.view([nb, nc, -1, nt]) / 2.0, dim=2
        )  # (bs, v, t)
        bd_z_square_error = (
            pred[:, :, :, :, 0, :] - target[:, :, :, :, 0, :]) ** 2
        bd_z_square_error += (pred[:, :, :, :, -1, :] -
                              target[:, :, :, :, -1, :]) ** 2
        bd_z_mean_square_error = torch.mean(
            bd_y_square_error.view([nb, nc, -1, nt]) / 2.0, dim=2
        )  # (bs, v, t)
        bd_mean_square_error = (
            bd_x_mean_square_error +
            bd_y_mean_square_error + bd_z_mean_square_error
        ) / 3.0
...
def L1RE(pred, target):
    ...
    nb, nc, nt = pred.shape[0], pred.shape[1], pred.shape[-1]
    l1_errors = torch.abs(
        pred.view([nb, nc, -1, nt]) - target.view([nb, nc, -1, nt]))
    res = torch.sum(l1_errors, dim=2) / torch.sum(
        torch.abs(target.view([nb, nc, -1, nt])), dim=2
    )
