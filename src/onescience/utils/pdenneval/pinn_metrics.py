...
def metric_func(pred, target, if_mean=True, Lx=1.0, Ly=1.0, Lz=1.0, iLow=4, iHigh=12):
    ...
    if len(idxs) == 4:  # 1D
        ...
        err_BD = (
            torch.sum(err_BD_x.reshape([nc, -1, nt]), dim=[-2, -1])
...
def bMSE(pred, target):
    ...
    if spatial_dim == 1:
        ...
        bd_mean_square_error = (bd_x_mean_square_error + bd_y_mean_square_error) / 2.0
    else:  # spatial_dim == 3
        bd_x_square_error = (pred[:, :, 0, :, :, :] - target[:, :, 0, :, :, :]) ** 2
        bd_x_square_error += (pred[:, :, -1, :, :, :] - target[:, :, -1, :, :, :]) ** 2
        bd_x_mean_square_error = torch.mean(
        ...
        bd_mean_square_error = (
            bd_x_mean_square_error + bd_y_mean_square_error + bd_z_mean_square_error
        ) / 3.0
