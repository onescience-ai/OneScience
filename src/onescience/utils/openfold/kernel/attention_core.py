...
try:
    ...
    # from onescience.utils.openfold.kernel.torch_ext_compile import compile
    from torch_ext_compile import compile
    current_dir = os.path.dirname(__file__)
    source_files = ["softmax_cuda.cpp", "softmax_cuda_kernel.cu"]
    sources = []
    for file in source_files:
        full_path = os.path.join(current_dir, "csrc", file)
    ...
    attn_core_inplace_cuda = compile(
        name="attn_core_inplace_cuda",
        sources=sources,
        extra_include_paths=[os.path.join(current_dir, "csrc")],
