...
from .variables import CategoricalVariable, Variable
...
class InputSpace(object):
    ...
    def get_variable_by_idx(self, idx: int) -> Variable:
        ...
        :type idx: int
        """
        return self.get_variable_by_name(self._variable_names[idx])
    ...
    def get_dict_from_array(self, x: np.ndarray) -> Dict[str, Union[str, float, int]]:
        """
...
def _latinhypercube_sample(
    rng: Union[int, Callable, np.random.RandomState], ndim: int, size: int
) -> np.ndarray:
    # generate row and column grids
    grid_bounds = np.stack(
        [np.linspace(0.0, 1.0, size + 1) for i in np.arange(ndim)], axis=1
    )
