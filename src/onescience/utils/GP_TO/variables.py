...
class NumericalVariable(Variable):
    ...
    def sample(self, random_state: np.random.RandomState, size: int) -> np.ndarray:
...
class CategoricalVariable(Variable):
    ...
    def __init__(self, name: str, levels: Union[List, Tuple]) -> None:
    ...
    def _transform(self, vector):
        return np.array([self.levels[int(x)] for x in vector])
    ...
    def _inverse_transform(self, vector: np.ndarray) -> np.ndarray:
        return np.array([self.levels.index(x) for x in vector])
