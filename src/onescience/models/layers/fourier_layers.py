            ...
            if "gaussian" in frequencies[0]:
                ...
                np_f = (
                    np.random.normal(0, 1, size=(
                        nr_freq, in_features)) * frequencies[1]
                )
            else:
                ...
                if "axis" in frequencies[0]:
                    ...
                    for i in range(in_features):
                        np_f_i[:, i, i] = np.reshape(
                            np.array(
                                frequencies[1]), (nr_freq)
                        )
                    np_f.append(
                        np.reshape(
                            np_f_i, (nr_freq * in_features, in_features))
                    )
                if "diagonal" in frequencies[0]:
                    np_f_i = np.reshape(
                        np.array(frequencies[1]), (nr_freq, 1, 1))
                    np_f_i = np.tile(
                        np_f_i, (1, in_features, in_features))
                    np_f_i = np.reshape(
                        np_f_i, (nr_freq * in_features, in_features))
                    np_f.append(np_f_i)
...
class FourierFilter(nn.Module):
    ...
    def __init__(
        ...
        self.weight_scale = input_scale / \
            math.sqrt(nr_layers + 1)

        self.frequency = nn.Parameter(
            torch.empty(in_features, layer_size))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_i = torch.sin(torch.matmul(
            x, 2.0 * math.pi * frequency) + self.phase)
        return x_i
    ...
    def reset_parameters(self) -> None:
        ...
        with torch.no_grad():
            self.gamma.copy_(
                torch.from_numpy(
                    np.random.gamma(
                        self.alpha, 1.0 / self.beta, (self.layer_size)),
                )
            )

    def forward(self, x: Tensor) -> Tensor:
        frequency = self.weight_scale * \
            (self.frequency * self.gamma.sqrt())
