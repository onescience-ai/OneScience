...
class Transolver2D(nn.Module):
    def __init__(
        ...
        if self.unified_pos:
            self.preprocess = MLP(
                fun_dim + space_dim + self.ref * self.ref,
                n_hidden * 2,
                n_hidden,
                n_layers=0,
                res=False,
    ...
    def get_grid(self, my_pos):
        ...
        gridx = torch.tensor(
            np.linspace(-2, 4, self.ref), dtype=torch.float)
        gridx = gridx.reshape(1, self.ref, 1, 1).repeat(
            [batchsize, 1, self.ref, 1])
        gridy = torch.tensor(
            np.linspace(-1.5, 1.5, self.ref), dtype=torch.float)
        gridy = gridy.reshape(1, 1, self.ref, 1).repeat(
            [batchsize, self.ref, 1, 1])
        grid_ref = (
            torch.cat((gridx, gridy), dim=-
                      1).cuda().reshape(batchsize, self.ref**2, 2)
        )  # B 4 4 4 3
        ...
        pos = (
            ...
            .reshape(batchsize, my_pos.shape[1], self.ref * self.ref)
            .contiguous()
        )
        return pos
