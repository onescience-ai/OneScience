...
class Linear(torch.nn.Module):
    ...
    def __init__(
        ...
        self.weight = torch.nn.Parameter(
            weight_init([out_features, in_features],
                        **init_kwargs) * init_weight
        )
        self.bias = (
            torch.nn.Parameter(weight_init(
                [out_features], **init_kwargs) * init_bias)
            if bias
            else None
        )
...
class Conv2d(torch.nn.Module):
    ...
    def __init__(
        ...
        if up and down:
            raise ValueError(
                "Both 'up' and 'down' cannot be true at the same time.")
        ...
        self.bias = (
            torch.nn.Parameter(weight_init(
                [out_channels], **init_kwargs) * init_bias)
            if kernel and bias
            else None
        )
        f = torch.as_tensor(
            resample_filter, dtype=torch.float32)
        f = f.ger(f).unsqueeze(0).unsqueeze(
            1) / f.sum().square()
        self.register_buffer(
            "resample_filter", f if up or down else None)

    def forward(self, x):
        w = self.weight.to(
            x.dtype) if self.weight is not None else None
        b = self.bias.to(
            x.dtype) if self.bias is not None else None
        f = (
        ...
        if self.fused_resample and self.up and w is not None:
            ...
            x = torch.nn.functional.conv2d(
                x, w, padding=max(w_pad - f_pad, 0))
        elif self.fused_resample and self.down and w is not None:
            x = torch.nn.functional.conv2d(
                x, w, padding=w_pad + f_pad)
            x = torch.nn.functional.conv2d(
                x,
...
class GroupNorm(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.num_groups = min(
            num_groups, num_channels // min_channels_per_group)
        self.eps = eps
        self.weight = torch.nn.Parameter(
    ...
    def forward(self, x):
        if self.training:
            ...
            x = x.float()
            x = rearrange(
                x, "b (g c) h w -> b g c h w", g=self.num_groups)
    ...
    def forward(ctx, q, k):
        w = (
            torch.einsum(
                ...
                q.to(torch.float32),
                (k /
                 torch.sqrt(torch.tensor(k.shape[1]))).to(torch.float32),
            )
            .softmax(dim=2)
            .to(q.dtype)
        )
...
class UNetBlock(torch.nn.Module):
    ...
    def forward(self, x, emb):
        ...
        params = self.affine(emb).unsqueeze(
            2).unsqueeze(3).to(x.dtype)
        if self.adaptive_scale:
            scale, shift = params.chunk(chunks=2, dim=1)
            x = silu(torch.addcmul(
                shift, self.norm1(x), scale + 1))
        else:
        ...
        x = x.add_(self.skip(orig)
                   if self.skip is not None else orig)
        x = x * self.skip_scale
        ...
        if self.num_heads:
            q, k, v = (
                ...
                .reshape(
                    x.shape[0] *
                    self.num_heads, x.shape[1] // self.num_heads, 3, -1
                )
                .unbind(2)
            )
...
class PositionalEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        freqs = freqs / (self.num_channels //
                         2 - (1 if self.endpoint else 0))
