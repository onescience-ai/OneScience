...
class UNO1d(nn.Module):
    def __init__(self, num_channels, width, pad=9, factor=1, initial_step=10):
        ...
        self.conv2 = OperatorBlock_1D(
            4 * factor * self.width, 4 * factor * self.width, 12, 5
        )

        self.conv4 = OperatorBlock_1D(
            4 * factor * self.width, 2 * factor * self.width, 24, 5, Normalize=True
        )

        self.conv5 = OperatorBlock_1D(
            4 * factor * self.width, self.width, 57, 11
        )  # will be reshaped
...
class UNO2d(nn.Module):
    def __init__(self, num_channels, width, pad=6, factor=1, initial_step=10):
        ...
        self.conv0 = OperatorBlock_2D(
            self.width, 2 * factor * self.width, 36, 36, 17, 17
        )
        ...
        self.conv2 = OperatorBlock_2D(
            4 * factor * self.width, 4 * factor * self.width, 18, 18, 7, 7
        )
        ...
        self.conv5 = OperatorBlock_2D(
            4 * factor * self.width, self.width, 81, 81, 17, 17
        )  # will be reshaped
...
class UNO3d(nn.Module):
    def __init__(self, num_channels, width, pad=5, factor=1, initial_step=10):
        ...
        self.conv0 = OperatorBlock_3D(
            self.width, 2 * factor * self.width, 36, 36, 36, 17, 17, 17
        )
        ...
        self.conv5 = OperatorBlock_3D(
            4 * factor * self.width, self.width, 81, 81, 81, 17, 17, 17
        )  # will be reshaped
...
class UNO_maxwell(nn.Module):
    def __init__(self, num_channels, width, pad=5, factor=1, initial_step=10):
        ...
        self.conv0 = OperatorBlock_3D(
            self.width, 2 * factor * self.width, 24, 24, 24, 10, 10, 10
        )
        ...
        self.conv5 = OperatorBlock_3D(
            4 * factor * self.width, self.width, 53, 53, 53, 10, 10, 10
        )  # will be reshaped
