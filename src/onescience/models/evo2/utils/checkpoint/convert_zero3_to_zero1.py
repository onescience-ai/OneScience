...
def convert_zero_checkpoint_to_fp32_state_dict(
    ...
    if ranks_to_process is not None:
        ...
        assert (
            len(ranks_to_process) <= mp_size
        ), f"Expected {mp_size} ranks to process, got {len(ranks_to_process)}"
        assert all(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing MP shards"
    )
    parser.add_argument(
        "-t",
        "--tag",
    ...
    parser.add_argument(
        ...
        required=True,
        type=int,
        help="Model parallel size of source checkpoint",
    )
    parser.add_argument(
        "--rank_start", default=None, type=int, help="Start rank to process"
    )
    parser.add_argument(
        "--rank_end", default=None, type=int, help="End rank to process"
    )
    parser.add_argument(
        "--num_workers",
        default=1,
    ...
    if args.rank_start is not None:
        if args.rank_end is None:
            ...
            assert (
                args.rank_end < args.mp_size
            ), "Expected end rank to be less than mp_size"
