...
def run(args) -> None:
    ...
    tools.setup_logger(level=args.log_level,
                       tag=tag, directory=args.log_dir, rank=rank)
    ...
    try:
        ...
        logging.info(
            "Cannot find MACE version, please install MACE via pip")
    ...
    if args.heads is not None:
        ...
        for _, head_dict in args.heads.items():
            ...
            update_keyspec_from_kwargs(
                head_keyspec, head_dict)
            head_keyspec.update(
                info_keys=head_dict.get("info_keys", {}),
                arrays_keys=head_dict.get(
                    "arrays_keys", {}),
            )
    ...
    logging.info(
        "Using the key specifications to parse data:")
    ...
    for head, head_args in args.heads.items():
        ...
        head_config = dict_head_to_dataclass(
            head_args, head, args)
        ...
        if hasattr(head_config, "test_file") and head_config.test_file is not None:
            head_config.test_file = normalize_file_paths(
                head_config.test_file)
        ...
        if head_config.train_file == ["mp"]:
            ...
            collections = assemble_mp_data(
                args, head_config, tag)
    ...
    if args.multiheads_finetuning:
        ...
        all_ase_readable = all(
            all(check_path_ase_read(f)
                for f in head_config.train_file)
            for head_config in head_configs
        )
        head_config_pt = filter(
            lambda x: x.head_name == "pt_head", head_configs)
    ...
    for head_config in head_configs:
        if head_config.atomic_numbers is None:
            assert all(check_path_ase_read(
                f) for f in head_config.train_file), "Must specify atomic_numbers when using .h5 or .aselmdb train_file input"
            ...
            if head_config.statistics_file is None:
                ...
                logging.info(
                    "Using atomic numbers from statistics file")
            zs_list = ast.literal_eval(
                head_config.atomic_numbers)
    ...
    z_table = AtomicNumberTable(
        sorted(list(all_atomic_numbers)))
    ...
    heads = sorted(heads, key=lambda x: -
                   1000 if x == "pt_head" else 0)
    ...
    if args.foundation_model_elements and model_foundation:
        ...
        for head_name, head_energies in atomic_energies_dict.items():
            energy_head_padded = {}
            for z in z_table.zs:
                energy_head_padded[z] = head_energies.get(
                    z, 0.0)
    ...
    if args.model == "AtomicDipolesMACE":
        ...
        # )
        atomic_energies = dict_to_array(
            atomic_energies_dict, heads)
        for head_config in head_configs:
            try:
                logging.info(f"Atomic Energies used (z: eV) for head {head_config.head_name}: " + "{" + ", ".join(
                    [f"{z}: {atomic_energies_dict[head_config.head_name][z]}" for z in head_config.z_table.zs]) + "}")
            except KeyError as e:
                raise KeyError(
                    f"Atomic number {e} not found in atomic_energies_dict for head {head_config.head_name}, add E0s for this atomic number") from e
    ...
    for head_config in head_configs:
        ...
        logging.info(
            f"Processing datasets for head '{head_config.head_name}'")
        ase_files = [
            f for f in head_config.train_file if check_path_ase_read(f)]
        non_ase_files = [
    ...
    train_loader = torch_geometric.dataloader.DataLoader(
        ...
        drop_last=(
            train_sampler is None and not args.lbfgs),
        pin_memory=args.pin_memory,
        num_workers=args.num_workers,
        generator=torch.Generator().manual_seed(args.seed),
    )
    valid_loaders = {
        heads[i]: None for i in range(len(heads))}
    ...
    logging.debug(model)
    logging.info(
        f"Total number of parameters: {tools.count_parameters(model)}")
    logging.info("")
    logging.info(
        "===========OPTIMIZER INFORMATION===========")
    logging.info(
        f"Using {args.optimizer.upper()} as parameter optimizer")
    ...
    if args.enable_cueq:
        logging.info(
            "Converting model to CUEQ for accelerated training")
        assert model.__class__.__name__ in [
            "MACE", "ScaleShiftMACE"]
        model = run_e3nn_to_cueq(
            deepcopy(model), device=device)
    # Optimizer
    ...
    if args.device == "xpu":
        logging.info(
            "Optimzing model and optimzier for XPU")
    ...
    if args.swa:
        swa, swas = get_swa(
            args, model, optimizer, swas, dipole_only)
    ...
    for swa_eval in swas:
        ...
        if rank == 0:
            ...
            if swa_eval:
                ...
                model_path = Path(
                    args.checkpoints_dir) / (tag + ".model")
            ...
            if swa_eval:
                torch.save(
                    model_to_save, Path(
                        args.model_dir) / (args.name + "_stagetwo.model")
                )
                try:
                ...
                torch.save(model_to_save, Path(
                    args.model_dir) / (args.name + ".model"))
                try:
                    path_complied = Path(args.model_dir) / (
        ...
        if skip_heads:
            logging.info(
                f"Skipping evaluation for heads: {skip_heads}")
