...
class BesselBasis(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        numerator = torch.sin(
            self.bessel_weights * x)  # [..., num_basis]
...
class PolynomialCutoff(torch.nn.Module):
    ...
    def calculate_envelope(
        ...
        envelope = (
            1.0
            - ((p + 1.0) * (p + 2.0) / 2.0) *
            torch.pow(r_over_r_max, p)
            + p * (p + 2.0) * torch.pow(r_over_r_max, p + 1)
            - (p * (p + 1.0) / 2) *
            torch.pow(r_over_r_max, p + 2)
        )
...
class ZBLBasis(torch.nn.Module):
    ...
    def __init__(self, p=6, trainable=False, **kwargs):
        ...
        if trainable:
            self.a_exp = torch.nn.Parameter(
                torch.tensor(0.300, requires_grad=True))
            self.a_prefactor = torch.nn.Parameter(
                torch.tensor(0.4543, requires_grad=True)
            )
        else:
            self.register_buffer(
                "a_exp", torch.tensor(0.300))
            self.register_buffer(
                "a_prefactor", torch.tensor(0.4543))
    ...
    def forward(
        ...
        r_max = self.covalent_radii[Z_u] + \
            self.covalent_radii[Z_v]
        envelope = PolynomialCutoff.calculate_envelope(
            x, r_max, self.p)
        v_edges = 0.5 * v_edges * envelope
        V_ZBL = scatter_sum(
            v_edges, receiver, dim=0, dim_size=node_attrs.size(0))
    ...
    def __init__(
        ...
        if trainable:
            self.a = torch.nn.Parameter(
                torch.tensor(1.0805, requires_grad=True))
            self.q = torch.nn.Parameter(
                torch.tensor(0.9183, requires_grad=True))
            self.p = torch.nn.Parameter(
                torch.tensor(4.5791, requires_grad=True))
...
class SoftTransform(torch.nn.Module):
    ...
    def compute_r_0(
        ...
        Z_u = node_atomic_numbers[sender]
        Z_v = node_atomic_numbers[receiver]
        r_0: torch.Tensor = self.covalent_radii[Z_u] + \
            self.covalent_radii[Z_v]
        return r_0
    ...
    def forward(
        ...
        r_0 = self.compute_r_0(
            node_attrs, edge_index, atomic_numbers)
        p_0 = (3 / 4) * r_0
        p_1 = (4 / 3) * r_0
        m = 0.5 * (p_0 + p_1)
