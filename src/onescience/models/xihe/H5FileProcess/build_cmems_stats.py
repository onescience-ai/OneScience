...
def calculate():
    ...
    for i in range(len(variables)):
        if os.path.exists(
            f"{OUTPUT_DIR}/{variables[i]}_global_means.npy"
        ) and os.path.exists(f"{OUTPUT_DIR}/{variables[i]}_global_stds.npy"):
            print(f"{variables[i]} has been calculated, skipping...")
>>>>>>> 09328ad50aa71561f4a4749cba9f63881eed2c84
            EXIST = -1
        else:
            EXIST = i
            break
...
=======
                for h5file in tqdm(
                    h5files, desc=f"{int(year)}/{variables[i]}", unit="files"
                ):
                    with h5py.File(os.path.join(DATA_DIR, str(year), h5file), "r") as f:
                        data = f["fields"][i : i + 1]
                        data = data[~np.isnan(data)]  # 去掉nan值
                        if isBegin:
                            sum_vals = np.sum(data)
                            sum_sq_vals = np.sum(data**2)
                            isBegin = 0
