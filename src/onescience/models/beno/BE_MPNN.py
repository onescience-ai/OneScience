...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.node_fn_inbd = nn.Sequential(
            *[
                build_mlp(
                    nnode_in_features,
                    [mlp_hidden_dim for _ in range(
        ...
        self.edge_fn_inbd = nn.Sequential(
            *[
                build_mlp(
                    nedge_in_features,
                    [mlp_hidden_dim for _ in range(
    ...
    def forward(
        ...
        return (
            self.node_fn(x),
            self.edge_fn(edge_features),
            self.node_fn_inbd(x_inbd),
            self.edge_fn_inbd(edge_inbd_features),
        )
...
class InteractionNetwork(MessagePassing):
    def __init__(
        ...
        # boundary
        self.boundary_fn = Transformer(
            enc_in=3, d_model=boundary_dim, n_heads=2, enc_layers=trans_layer
        )
    ...
    def forward(
        ...
        for gnn in self.gnn_stacks_inbd:
            x_inbd, edge_inbd_features = gnn(
                x_inbd, edge_inbd_index, edge_inbd_features, boundary_inbd
            )
...
class Decoder(nn.Module):
    ...
    def forward(self, x: torch.tensor, x_inbd: torch.tensor):
        u1 = self.node_fn(x)
...
class HeteroGNS(nn.Module):
    ...
    def forward(self, data):
        ...
        x_inbd, edge_inbd_index, edge_inbd_features, boundary_inbd = (
            data["G2"].x,
            data["G2"].edge_index,
            data["G2"].edge_features,
            data["G2"].boundary,
        )

        x, edge_features, x_inbd, edge_inbd_features = self._encoder(
            x, edge_features, x_inbd, edge_inbd_features
        )

        x, edge_features, x_inbd, edge_inbd_features = self._processor(
            x,
            edge_index,
            edge_features,
            boundary,
