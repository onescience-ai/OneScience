...
class MLP(nn.Module):
    def __init__(
        self, input_size, output_size=128, layer_norm=True, n_hidden=2, hidden_size=128
    ):
...
class GNN(nn.Module):
    ...
    def forward(self, V, E, edges):
        senders = torch.gather(
            V, -
            2, edges[...,
                     0].unsqueeze(-1).repeat(1, 1, V.shape[-1])
        )
        receivers = torch.gather(
            V, -
            2, edges[...,
                     1].unsqueeze(-1).repeat(1, 1, V.shape[-1])
        )

        edge_inpt = torch.cat(
            [senders, receivers, E], dim=-1)
        edge_embeddings = self.f_edge(edge_inpt)

        col = edges[..., 0].unsqueeze(-1).repeat(
            1, 1, edge_embeddings.shape[-1])
...
class multiHeadGAT(nn.Module):
    def __init__(self, node_size, output_size, n_heads):
        ...
        self.gat = nn.ModuleList(
            [GAT(node_size, output_size // n_heads)
...
class GAT(nn.Module):
    def __init__(self, node_size, output_size):
        ...
        self.f_edge = nn.Linear(
            in_features=node_size, out_features=output_size, bias=False
        )
        self.attention = nn.Sequential(
            nn.Linear(in_features=output_size *
                      2 + 128, out_features=1),
            nn.LeakyReLU(0.2),
        )
    ...
    def forward(self, V, E, edges):
        ...
        attention = self.attention(
            torch.cat([h_sender, h_receiver, E], dim=-1))
        attention = torch.exp(
            attention - torch.max(attention, dim=1, keepdim=True)[0])
        col = edges[..., 0].unsqueeze(-1)

        numerator = scatter_sum(
            attention * h_sender, col.repeat(1, 1, h_sender.shape[-1]), dim=-2
        )
        denominator = scatter_sum(
            attention, col.repeat(1, 1, attention.shape[-1]), dim=-2
        )
...
class Normalizer(nn.Module):
    def __init__(self, input_size):
        ...
        self.accumulation = nn.Parameter(
            torch.zeros(input_size), requires_grad=False)
        self.accumulation_squarred = nn.Parameter(
            torch.zeros(input_size), requires_grad=False
        )
        self.mean = nn.Parameter(torch.zeros(
            input_size), requires_grad=False)
        self.std = nn.Parameter(torch.zeros(
            input_size), requires_grad=False)
    ...
    def forward(self, x):
        ...
        if self.training is True:
            if self.count < self.max_accumulation:
                ...
                self.accumulation.data = self.accumulation.data + torch.mean(
                    x, dim=(0, 1)
                )
                self.accumulation_squarred.data = (
                    self.accumulation_squarred.data +
                    torch.mean(x**2, dim=(0, 1))
                )

                self.mean.data = self.accumulation / \
                    (self.count + 1e-8)
                self.std.data = torch.sqrt(
                    self.accumulation_squarred /
                    (self.count + 1e-8) - self.mean.data**2
                )
