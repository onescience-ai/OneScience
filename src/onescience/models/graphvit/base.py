...
class GNN(nn.Module):
    ...
    def forward(self, V, E, edges):
        senders = torch.gather(
            V, -2, edges[..., 0].unsqueeze(-1).repeat(1, 1, V.shape[-1])
        )
        receivers = torch.gather(
            V, -2, edges[..., 1].unsqueeze(-1).repeat(1, 1, V.shape[-1])
        )
...
class multiHeadGAT(nn.Module):
    def __init__(self, node_size, output_size, n_heads):
        ...
        self.gat = nn.ModuleList(
            [GAT(node_size, output_size // n_heads) for _ in range(n_heads)]
        )
...
class GAT(nn.Module):
    def __init__(self, node_size, output_size):
        ...
        self.attention = nn.Sequential(
            nn.Linear(in_features=output_size * 2 + 128, out_features=1),
            nn.LeakyReLU(0.2),
        )
        ...
        if self.training is True:
            if self.count < self.max_accumulation:
                ...
                self.accumulation_squarred.data = (
                    self.accumulation_squarred.data + torch.mean(x**2, dim=(0, 1))
                )

                self.mean.data = self.accumulation / (self.count + 1e-8)
                self.std.data = torch.sqrt(
                    self.accumulation_squarred / (self.count + 1e-8) - self.mean.data**2
                )
