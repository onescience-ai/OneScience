...
from onescience.models.figconvunet.components.mlp import MLPBlock
from onescience.models.figconvunet.components.reductions import (
    REDUCTION_TYPES,
    row_reduction,
)
from onescience.models.figconvunet.geometries import PointFeatures


from onescience.models.figconvunet.neighbor_ops import batched_neighbor_knn_search
...
class PointFeatureConv(nn.Module):
    ...
    def __init__(
        ...
        if neighbor_search_type == "knn":
            assert knn_k is not None
            self.radius_or_k = knn_k
        else:
            raise ValueError(
                f"neighbor_search_type must be knn, got {neighbor_search_type}"
            )
    ...
    def forward(
        ...
        if self.neighbor_search_type == "knn":
            ...
            raise ValueError(
                f"neighbor_search_type must be knn, got {self.neighbor_search_type}"
            )
