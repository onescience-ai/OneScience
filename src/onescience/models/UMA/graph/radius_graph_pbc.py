...
def sum_partitions(x: torch.Tensor, partition_idxs: torch.Tensor) -> torch.Tensor:
    sums = torch.zeros(
        partition_idxs.shape[0] - 1, device=x.device, dtype=x.dtype)
    for idx in range(partition_idxs.shape[0] - 1):
        sums[idx] = x[partition_idxs[idx]
            : partition_idxs[idx + 1]].sum()
    return sums
...
def get_counts(x: torch.Tensor, length: int):
    ...
    return torch.zeros(length, device=device, dtype=dtype).scatter_reduce(
        ...
        index=x,
        src=torch.ones(
            x.shape[0], device=device, dtype=dtype),
        reduce="sum",
    )
...
def compute_neighbors(data, edge_index):
    ...
    num_neighbors = get_counts(
        edge_index[1], data.natoms.sum())
...
def get_max_neighbors_mask(
    ...
    index_neighbor_offset = torch.cumsum(
        num_neighbors, dim=0) - num_neighbors
    ...
    distance_sort.index_copy_(
        0, index_sort_map, atom_distance)
    distance_sort = distance_sort.view(
        num_atoms, max_num_neighbors)

    # Sort neighboring atoms based on distance
    distance_sort, index_sort = torch.sort(
        distance_sort, dim=1)
    ...
    if enforce_max_strictly:
        ...
        is_included = torch.le(
            distance_sort.T, effective_cutoff)
        ...
        num_included_per_atom = torch.sum(
            is_included, dim=0)
    ...
    index_sort = torch.masked_select(
        index_sort, mask_finite)

    # At this point index_sort contains the index into index of the
    # closest max_num_neighbors_threshold neighbors per atom
    # Create a mask to remove all pairs not in index_sort
    mask_num_neighbors = torch.zeros(
        len(index), device=device, dtype=bool)
...
def radius_graph_pbc(
    ...
    num_atoms_per_image_sqr = (
        num_atoms_per_image**2).long()

    # index offset between images
    index_offset = torch.cumsum(
        num_atoms_per_image, dim=0) - num_atoms_per_image
    ...
    index_sqr_offset = (
        torch.cumsum(num_atoms_per_image_sqr,
                     dim=0) - num_atoms_per_image_sqr
    )
    index_sqr_offset = torch.repeat_interleave(
        index_sqr_offset, num_atoms_per_image_sqr
    )
    atom_count_sqr = torch.arange(
        num_atom_pairs, device=device) - index_sqr_offset
    ...
    index2 = (atom_count_sqr %
              num_atoms_per_image_expand) + index_offset_expand
    ...
    cross_a2a3 = torch.cross(
        data.cell[:, 1], data.cell[:, 2], dim=-1)
    cell_vol = torch.sum(
        data.cell[:, 0] * cross_a2a3, dim=-1, keepdim=True)

    if pbc[0]:
        inv_min_dist_a1 = torch.norm(
            cross_a2a3 / cell_vol, p=2, dim=-1)
    ...
    if pbc[1]:
        cross_a3a1 = torch.cross(
            data.cell[:, 2], data.cell[:, 0], dim=-1)
        inv_min_dist_a2 = torch.norm(
            cross_a3a1 / cell_vol, p=2, dim=-1)
    ...
    if pbc[2]:
        cross_a1a2 = torch.cross(
            data.cell[:, 0], data.cell[:, 1], dim=-1)
        inv_min_dist_a3 = torch.norm(
            cross_a1a2 / cell_vol, p=2, dim=-1)
    ...
    cells_per_dim = [
        torch.arange(-rep.item(), rep.item() + 1,
                     device=device, dtype=data.cell.dtype)
        for rep in max_rep
    ]
    unit_cell = torch.cartesian_prod(*cells_per_dim)
    num_cells = len(unit_cell)
    unit_cell_per_atom = unit_cell.view(
        1, num_cells, 3).repeat(len(index2), 1, 1)
    unit_cell = torch.transpose(unit_cell, 0, 1)
    unit_cell_batch = unit_cell.view(
        1, 3, num_cells).expand(batch_size, -1, -1)
    ...
    index1 = index1.view(-1, 1).repeat(1,
                                       num_cells).view(-1)
    index2 = index2.view(-1, 1).repeat(1,
                                       num_cells).view(-1)
    ...
    mask_within_radius = torch.le(
        atom_distance_sqr, radius * radius)
    # Remove pairs with the same atoms (distance = 0.0)
    mask_not_same = torch.gt(atom_distance_sqr, 0.0001)
    mask = torch.logical_and(
        mask_within_radius, mask_not_same)
    index1 = torch.masked_select(index1, mask)
    index2 = torch.masked_select(index2, mask)
    unit_cell = torch.masked_select(
        unit_cell_per_atom.view(-1,
                                3), mask.view(-1, 1).expand(-1, 3)
    )
    unit_cell = unit_cell.view(-1, 3)
    atom_distance_sqr = torch.masked_select(
        atom_distance_sqr, mask)
    ...
    if not torch.all(mask_num_neighbors):
        ...
        index1 = torch.masked_select(
            index1, mask_num_neighbors)
        index2 = torch.masked_select(
            index2, mask_num_neighbors)
        unit_cell = torch.masked_select(
            unit_cell.view(-1, 3), mask_num_neighbors.view(-1,
                                                           1).expand(-1, 3)
        )
        unit_cell = unit_cell.view(-1, 3)
...
def canonical_pbc(data, pbc: torch.Tensor | None):
    assert hasattr(
        data, "pbc"), "AtomicData does not have pbc set"
...
def radius_graph_pbc_v2(
    ...
    cell_matrix = torch.repeat_interleave(
        cell_matrix, cells_per_image, dim=0)
    pbc_cell_offsets = torch.bmm(
        cell_matrix, unit_cell.view(-1, 3, 1)).squeeze(-1)
    ...
    source_atom_index = torch.repeat_interleave(
        torch.arange(
            num_atoms, device=device).long(), num_cells_per_atom
    )
    ...
    for i in range(batch_size):
        cell_indices = (
            torch.arange(offset, offset +
                         cells_per_image[i], device=device)
            .repeat(num_atoms_per_image[i])
            .long()
        )
        pbc_cell_index = torch.cat(
            [pbc_cell_index, cell_indices], dim=0)
        offset = offset + cells_per_image[i]
    ...
    max_internal_cell = (
        max(source_atom_pos.abs().max(),
            target_atom_pos.abs().max()) / grid_resolution
    )
    if max_internal_cell > 200:
        ...
        grid_resolution = max(
            grid_resolution,
            max(source_atom_pos.abs().max(),
                target_atom_pos.abs().max()) / 200,
        )
    source_atom_grid = torch.floor(
        source_atom_pos / grid_resolution).long()
    target_atom_grid = torch.floor(
        target_atom_pos / grid_resolution).long()
    ...
    max_num_source_atoms = torch.max(
    ...
    source_atom_mapping = (
        torch.arange(len(source_atom_offset_per_image),
                     device=device, dtype=torch.long)
        - source_atom_offset_per_image
    )
    source_atom_mapping = (
        source_atom_mapping
        + torch.repeat_interleave(unique_atom_image,
                                  num_source_atoms_per_image, 0)
    ...
    grid_min, no_op = torch.min(
        source_atom_grid_per_image, dim=1)
    grid_max, no_op = torch.max(
        source_atom_grid_per_image, dim=1)
    ...
    source_atom_grid = source_atom_grid - \
        grid_min[source_atom_image]
    target_atom_grid = target_atom_grid - \
        grid_min[target_atom_image]
    ...
    source_atom_grid_id = source_atom_grid_id + \
        source_atom_grid_offset
    ...
    target_atom_grid_id = target_atom_grid_id + \
        target_atom_grid_offset
    ...
    sort_grid_id, sort_indices = torch.sort(
        source_atom_grid_id)
    ...
    grid_cell_atom_count.index_add_(
        0, source_atom_grid_id, torch.ones_like(
            source_atom_grid_id)
    )

    # Maximum number of atoms in a grid cell used to pad the array of atoms in each
    max_atoms_per_grid_cell = torch.max(
        grid_cell_atom_count)

    # Compute a mapping from the grid cell lists to the atoms in that grid cell
    cum_sum_grid_cell_atom_count = torch.cumsum(
        grid_cell_atom_count, 0)
    cum_sum_grid_cell_atom_count = torch.roll(
        cum_sum_grid_cell_atom_count, 1, 0)
    ...
    grid_cell_offset = (  # If this OOMs it could be because boxsize is large and PBC is on
        torch.arange(num_grid_cells, device=device) *
    ...
    grid_atom_index = grid_atom_index.view(
        num_grid_cells, max_atoms_per_grid_cell)
    # Add a Null grid cell to the end
    grid_atom_index = torch.cat(
        [
            grid_atom_index,
            torch.zeros(max_atoms_per_grid_cell, device=device).view(
                1, -1).long() - 1,
        ],
        dim=0,
    )
    ...
    num_padding_grid_cells = (
        (2 * padding_size + 1) *
        (2 * padding_size + 1) * (2 * padding_size + 1)
    )

    padding_offsets = torch.arange(
        -padding_size, padding_size + 1, device=device, dtype=torch.long
    )
    padding_offsets = padding_offsets.view(
        1, -1).repeat(3, 1)
    padding_offsets = torch.cartesian_prod(
        padding_offsets[0, :], padding_offsets[1,
                                               :], padding_offsets[2, :]
    )
    padding_offsets = padding_offsets.unsqueeze(
        0).repeat(batch_size, 1, 1)

    grid_index = target_atom_grid.view(
        -1, 1, 3).repeat(1, num_padding_grid_cells, 1)
    grid_index = grid_index + \
        padding_offsets[target_atom_image]
    max_index = target_atom_grid_size.view(-1, 1, 3).expand(
        -1, num_padding_grid_cells, -1
    )
    out_of_bounds = torch.logical_and(
        grid_index.ge(0), grid_index.lt(max_index))
    out_of_bounds = torch.all(out_of_bounds, dim=2).ne(True)

    padding_offsets[:, :, 1] = padding_offsets[:,
                                               :, 1] * grid_size.unsqueeze(-1)[:, 0]
    padding_offsets[:, :, 2] = (
        padding_offsets[:, :, 2]
    ...
    neighboring_grid_cells = neighboring_grid_cells + \
        padding_offsets[target_atom_image]

    # Filter neighboring cells that are out of bounds
    neighboring_grid_cells.masked_fill_(
        out_of_bounds, null_grid_index)
    ...
    source_atom_edge_index = torch.masked_select(
        source_atom_edge_index, atom_pad_mask)
    target_atom_edge_index = torch.masked_select(
        target_atom_edge_index, atom_pad_mask)
    ...
    within_radius_mask = torch.logical_and(
        atom_distance_sqr.le(
            radius * radius), atom_distance_sqr.ne(0.0)
    )
    source_atom_edge_index = torch.masked_select(
    ...
    atom_distance_sqr = torch.masked_select(
        atom_distance_sqr, within_radius_mask)
    ...
    if not torch.all(mask_num_neighbors):
        # Mask out the atoms to ensure each atom has at most max_num_neighbors_threshold neighbors
        target_idx = torch.masked_select(
            target_idx, mask_num_neighbors)
        source_idx = torch.masked_select(
            source_idx, mask_num_neighbors)
        source_cell = torch.masked_select(
            source_cell.view(-1, 3), mask_num_neighbors.view(-1,
                                                             1).expand(-1, 3)
        )
