...
class SIREN(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.layers = [
            nn.Linear(num_in_features, layers[0]), Sine(w0=w0)]

        for index in range(len(layers) - 1):
            self.layers.extend(
                [nn.Linear(layers[index],
                           layers[index + 1]), Sine(w0=1)]
            )
...
class SINESmearing(nn.Module):
    def __init__(
        ...
        self.freq_filter = nn.Parameter(
            freq.view(-1, 1).repeat(1,
                                    num_in_features).view(1, -1),
            requires_grad=False,
        )
    ...
    def __init__(
        ...
        if basis_type == "powersine":
            ...
            self.smearing = torch.nn.Sequential(
                torch.nn.Linear(
                    num_in_features, num_freqs * num_in_features),
                Act(act),
            )
            self.out_dim = num_in_features * num_freqs
...
class SphericalSmearing(nn.Module):
    ...
    def __init__(self, max_n: int = 10, option: str = "all") -> None:
        ...
        self.out_dim = int(
            np.sum(self.m == 0) + 2 * np.sum(self.m != 0))
    ...
    def forward(self, xyz: torch.Tensor) -> torch.Tensor:
        ...
        theta_tile = np.tile(theta.reshape(
            len(xyz), 1), (1, len(self.m)))
        phi_tile = np.tile(phi.reshape(
            len(xyz), 1), (1, len(self.m)))

        harm = sph_harm(
            m_tile, n_tile, theta_tile, phi_tile)

        harm_mzero = harm[:, self.m == 0]
        harm_mnonzero = harm[:, self.m != 0]

        harm_real = np.concatenate(
            [harm_mzero.real, harm_mnonzero.real,
                harm_mnonzero.imag], axis=1
        )
