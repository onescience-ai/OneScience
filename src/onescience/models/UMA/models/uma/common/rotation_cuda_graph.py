...
class RotMatWignerCudaGraph:
    def __init__(self):
        ...
        ), "Cuda Graphs can only be used with GPUs"
        # lazy graph capture
        self.graph_mod = None
        ...
        logging.warning(
            "Using Cuda graphs for wigner matrix creation")
    ...
    def get_rotmat_and_wigner(
        ...
        if self.graph_mod is None or edge_dist_vec.shape[0] > self.max_edge_size:
            ...
            pad_n = self.max_edge_size - \
                edge_dist_vec.shape[0]
            input_padded = torch.nn.functional.pad(
                edge_dist_vec, (0, 0, 0, pad_n))
        ...
        out = self.graph_mod(
            input_padded, jds, x_hat, mask, neg_mask)

        edge_rot_mat = torch.narrow(
            out[0], 0, 0, edge_dist_vec.shape[0])
        wigner = torch.narrow(
            out[1], 0, 0, edge_dist_vec.shape[0])
        wigner_inv = torch.narrow(
            out[2], 0, 0, edge_dist_vec.shape[0])
...
def capture_rotmat_and_wigner_with_make_graph_callable(
    ...
    with torch.cuda.stream(s):
        ...
        mask, neg_mask = create_masks(
            edge_dist_vec_clone, x_hat)
...
def edge_rot_and_wigner_graph_capture_region(
    ...
    alpha, beta, gamma = euler_from_edge_rot_mat(
        edge_rot_mat, x_hat)
    wigner = eulers_to_wigner(
        alpha, beta, gamma, 0, lmax, Jd_buffers)
...
def init_edge_rot_mat_cuda_graph(
    ...
    norm_x = torch.where(
        mask.unsqueeze(
            1).expand(-1, 3), x_hat.expand_as(norm_x), norm_x
    )
    norm_x = torch.where(
        neg_mask.unsqueeze(
            1).expand(-1, 3), -x_hat.expand_as(norm_x), norm_x
