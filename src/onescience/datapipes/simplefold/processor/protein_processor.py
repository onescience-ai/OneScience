...
import torch
import numpy as np
from onescience.utils.simplefold.esm_utils import (
    af2_idx_to_esm_idx,
    compute_language_model_representations,
    batch_encode_sequences,
)
from onescience.utils.simplefold.boltz_utils import center_random_augmentation as torch_center_random

try:
    import mlx.core as mx
    from onescience.utils.simplefold.mlx_utils import center_random_augmentation as mlx_center_random
except:
    pass
...
class ProteinDataProcessor:
    ...
    def process_esm(
        ...
        B_out = B * multiplicity if multiplicity > 1 else B
        esm_s = torch.zeros(
            (B_out, L, esm_model.num_layers + 1, esm_s_.shape[-1]),
            device=self.device,
        )
        ...
        if multiplicity > 1:
            # Process all B sequences and repeat each one multiplicity times
            for i in range(B):
                true_len = true_mask[i].sum()
                assert true_len == num_tokens[i], f"Length mismatch: {true_len} != {num_tokens[i]}"
                # Repeat each sequence multiplicity times
                for j in range(multiplicity):
                    esm_s[i * multiplicity + j, :true_len] = esm_s_[i, true_mask[i]]
        else:
            # Process all B sequences without repetition
            for i in range(B):
                true_len = true_mask[i].sum()
                assert true_len == num_tokens[i], f"Length mismatch: {true_len} != {num_tokens[i]}"
    ...
    def batch_to_device(self, batch, multiplicity=1):
        for k, v in batch.items():
            if isinstance(v, torch.Tensor):
                # Skip esm_s if it's already been processed with multiplicity in process_esm
                if k == "esm_s" and multiplicity > 1:
                    # esm_s is already [B * multiplicity, ...], just move to device
    ...
    def preprocess_training(self, batch, esm_model=None, esm_dict=None, af2_to_esm=None):
        ...
        # Process ESM before batch_to_device to avoid dimension mismatch
        # because aa_seq is a list of strings (not expanded by batch_to_device)
        # process_esm will handle multiplicity internally
