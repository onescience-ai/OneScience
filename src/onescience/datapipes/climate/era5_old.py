...
class  ERA5Dataset(BaseDataset):
    ...
    def _init_split(self):
        ...
        if not isinstance(selected_years, (list, tuple, set)):
            raise ValueError(
                f"❌ ERA5 {self.mode} split must be an explicit year list/tuple/set. "
        ...
        if not self.selected_years:
            raise ValueError(
                f"❌ ERA5 {self.mode} split is empty. "
    ...
    def _init_files(self):
        ...
        for year in self.selected_years:
            ...
            if not files:
                raise ValueError(f"❌ No ERA5 files found for year {year} under {path}")
