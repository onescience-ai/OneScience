...
class GraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.valid_list is None:
            return (
                self.datalist[idx].pos,
                self.datalist[idx].x,
                self.datalist[idx].y,
                self.datalist[idx].surf,
                data.edge_index,
            )
        else:
            return (
...
class GraphDataset_mgn(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        distance = torch.norm(
            displacement, p=2, dim=1, keepdim=True)
        edge_features = torch.cat(
            [displacement, torch.log(distance + 1e-8)], dim=1)
        ...
        return Data(
            ...
            # 添加对象文件路径
            obj_file=self.obj_files[idx] if self.obj_files else "",
        )
