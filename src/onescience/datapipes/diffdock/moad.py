...
from onescience.utils.diffdock.utils import read_strings_from_txt
...
class MOAD(Dataset):
    def __init__(
        ...
        self.lig_cache_path = os.path.join(
            ...
            + ("" if not matching else "_matching")
            + ("" if not skip_matching else "skip")
        ...
        if remove_pdbbind:
            complexes_pdbbind = read_strings_from_txt("data/splits/timesplit_no_lig_overlap_train") + read_strings_from_txt(
    ...
    def get_by_name(self, ligand_name, cluster):
        ...
        for type_name in ligand_graph.node_types + ligand_graph.edge_types:
            for key, value in ligand_graph[type_name].items():
                complex_graph[type_name][key] = value
        ...
        if self.chain_cutoff:
            distances = torch.norm(
                (torch.from_numpy(complex_graph["ligand"].orig_pos[0]) - complex_graph.original_center).unsqueeze(1)
            ...
            if self.all_atoms:
                ...
                atom_res_edge_index = torch.stack(
                    [torch.arange(len(atom_to_res_new_mapping)), atom_to_res_new_mapping]
                )
    ...
    def get(self, idx):
        ...
        ligand_name = sorted(self.cluster_to_ligands[cluster])[0] if self.no_randomness else random.choice(
    ...
    def get_ligand(self, name):
        ...
        if self.max_lig_size is not None and lig.GetNumHeavyAtoms() > self.max_lig_size:
            print(
                f"Ligand with {lig.GetNumHeavyAtoms()} heavy atoms is larger than max_lig_size {self.max_lig_size}. "
...
def print_statistics(dataset):
    ...
    for i in range(len(dataset)):
        ...
        radius_molecule = torch.max(torch.linalg.vector_norm(lig_pos - molecule_center.unsqueeze(0), dim=1))
