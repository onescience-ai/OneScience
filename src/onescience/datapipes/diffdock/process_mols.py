...
from onescience.datapipes.diffdock.conformer_matching import (
    get_torsion_angles,
    optimize_rotatable_bonds,
)
from onescience.datapipes.diffdock.constants import aa_short2long, atom_order, three_to_one
from onescience.datapipes.diffdock.parse_chi import (
    aa_idx2aa_short,
    get_chi_angles,
    get_coords,
    get_onehot_sequence,
...
from onescience.utils.diffdock.torsion import get_transformation_mask
...
def new_extract_receptor_structure(
    ...
    complex_graph["receptor"].x = (
        torch.cat([node_feat, lm_embeddings], axis=1) if lm_embeddings is not None else node_feat
    )
...
def get_rec_misc_atom_feat(bio_atom=None, atom_name=None, element=None, get_misc_features=False):
    ...
    atom_feat = [
        safe_index(allowable_features["possible_amino_acids"], bio_atom.get_parent().get_resname()),
        safe_index(allowable_features["possible_atomic_num_list"], atomic_num),
        safe_index(allowable_features["possible_atom_type_2"], (atom_name + "*")[:2]),
        safe_index(allowable_features["possible_atom_type_3"], atom_name),
    ]
