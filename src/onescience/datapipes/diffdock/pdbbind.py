...
from onescience.utils.diffdock.diffusion_utils import modify_conformer, set_time
from onescience.utils.diffdock.utils import crop_beyond, read_strings_from_txt
...
class PDBBind(Dataset):
    def __init__(
        ...
        self.full_cache_path = os.path.join(
            ...
            + ("" if not self.fixed_knn_radius_graph else ("_fixedKNN" if not self.knn_only_graph else "_fixedKNNonly"))
        ...
        list_names = [complex_graph["name"] for complex_graph in self.complex_graphs]
        with open(
            os.path.join(
    ...
    def inference_preprocessing(self):
        ...
        for i in list_indices:
            ...
            with tqdm(total=len(protein_paths_chunk), desc=f"loading complexes {i}/{len(protein_paths_chunk)//1000+1}") as pbar:
                ...
                for t in map_fn(
                    self.get_complex,
                    zip(protein_paths_chunk, lm_embeddings_chains, ligands_chunk, ligand_description_chunk),
                ):
                    complex_graphs.extend(t[0])
                    rdkit_ligands.extend(t[1])
                    pbar.update()
    ...
    def get_complex(self, par):
        ...
        try:
            ...
            moad_extract_receptor_structure(
                path=os.path.join(self.pdbbind_dir, name, f"{name}_{self.protein_file}.pdb"),
