...
def make_atom14_masks_np(batch):
    batch = tree_map(lambda n: torch.tensor(
        n, device="cpu"), batch, np.ndarray)
...
def make_atom14_positions(protein):
    ...
    residx_atom14_gt_positions = residx_atom14_gt_mask[..., None] * (
        batched_gather(
            ...
            dim=-2,
            no_batch_dims=len(
                protein["all_atom_positions"].shape[:-2]),
        )
    )
    ...
    restype_3 = [rc.restype_1to3[res]
                 for res in rc.restypes]
    restype_3 += ["UNK"]
