...
def atom_array_to_input_json(
    ...
    unique_label_entity_id = np.unique(
    ...
    copy_id = np.vectorize(chain_id_to_copy_id_dict.get)(
        atom_array.chain_id)
    ...
    for label_entity_id in unique_label_entity_id:
        ...
        entity_type = parser.entity_poly_type.get(
            label_entity_id, "ligand")
        if entity_type != "ligand":
            ...
            # ligand
            lig_ccd = "_".join(
                label_entity_id_to_sequences[label_entity_id])
        ...
        all_entity_counts[entity_id_in_json] = len(
            asym_chains)
        if save_entity_and_asym_id:
            entity_dict["label_entity_id"] = str(
                label_entity_id)
    ...
    lig_lig_bonds = get_lig_lig_bonds(
        atom_array, lig_include_ions=False)
    inter_entity_bonds = np.vstack(
        (lig_polymer_bonds, lig_lig_bonds))

    lig_indices = np.where(
        np.isin(atom_array.chain_id, lig_chain_ids))[0]
    lig_bond_mask = np.any(
        np.isin(inter_entity_bonds[:, :2], lig_indices), axis=1)
    # select bonds of ligands
...
def cif_to_input_json(
    ...
    if assembly_id is not None:
        ...
        atom_array = parser.expand_assembly(
...
if __name__ == "__main__":
    ...
    print(cif_to_input_json(
        args.cif_file, output_json=args.json_file))
