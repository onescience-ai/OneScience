...
def mha(
    ...
    if mask is not None:
        ...
        logits = jax.vmap(jax.vmap(apply_mask, in_axes=[None, 0]), in_axes=[None, 0])(
            mask, logits
        )
...
def _make_splash_mha(
    ...
    attn = splash_attention.make_splash_mha(
        mask,
...
def layernorm(x: jnp.ndarray, create_scale: bool, create_offset: bool) -> jnp.ndarray:
    return hk.LayerNorm(
        axis=-1, create_scale=create_scale, create_offset=create_offset, name="norm"
    )(x)


def mask_block_diags(
    mask: sp.sparse.csr_matrix, num_padding_nodes: int, block_size: int
) -> jnp.ndarray:
