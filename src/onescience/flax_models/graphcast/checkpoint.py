...
def _convert_types(typ: type[_T], value: Any) -> _T:
    ...
    if dataclasses.is_dataclass(typ):
        ...
        for f in dataclasses.fields(typ):
            ...
            if isinstance(f.type, (types.UnionType, type(Optional[int]))):
                constructors = [
                    t for t in f.type.__args__ if t is not types.NoneType]
            ...
            if f.name in value:
                kwargs[f.name] = _convert_types(
                    constructor, value[f.name])
            else:
    ...
    if base_type is dict:
        ...
        return {
            _convert_types(key_type, k): _convert_types(value_type, v)
    ...
    if base_type is tuple:
        if len(typ.__args__) == 2 and typ.__args__[1] == ...:
            ...
            return tuple(
                ...
                for t, (_, v) in zip(
                    typ.__args__, sorted(
                        value.items(), key=lambda x: int(x[0]))
                )
            )
