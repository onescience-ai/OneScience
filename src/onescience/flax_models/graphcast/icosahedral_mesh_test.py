...
class IcosahedralMeshTest(parameterized.TestCase):
    ...
    def test_merge_meshes(self, splits):
        mesh_hierarchy = icosahedral_mesh.get_hierarchy_of_triangular_meshes_for_sphere(
            splits=splits
        )
...
def _assert_positive_face_orientation(vertices, faces):
    ...
    # exactly parallel.
    np.testing.assert_allclose(
        dot_center_orientation, 1.0, atol=6e-4)
