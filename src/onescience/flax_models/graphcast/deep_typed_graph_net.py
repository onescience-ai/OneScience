...
class DeepTypedGraphNet(hk.Module):
    ...
    * num_message_passing_steps = N, num_processor_repetitions = M, gives
    ...
    def __init__(
        ...
        *,
        ...
        if use_norm_conditioning and not use_layer_norm:
            raise ValueError(
                "`norm_conditioning` can only be used when " "`use_layer_norm` is true."
            )
    ...
    def __call__(
        ...
        embedder_network, processor_networks, decoder_network = self._networks_builder(
    ...
    def _networks_builder(
        ...
        global_norm_conditioning: Optional[chex.Array] = None,
    ) -> Tuple[GraphToGraphNetwork, List[GraphToGraphNetwork], GraphToGraphNetwork]:
