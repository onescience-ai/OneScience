...
from onescience.flax_models.MolSculptor.train.rewards import (
    dsdp_batch_reward,
    tanimoto_sim,
)
...
def dual_inhibitor_reward_function(
    molecule_dict, cached_dict, dsdp_script_paths, save_path
):
    ...
    todo_unique_smiles = np.setdiff1d(
        todo_unique_smiles, unique_smiles)
    ...
    if todo_unique_smiles.size > 0:
        ...
        r_dock_2 = np.asarray(
            r_dock_2, np.float32) * (-1.0)  # (N,)
        todo_unique_scores = np.stack(
            [r_dock_1, r_dock_2], axis=1)  # (N, 2)
        unique_smiles = np.concatenate(
            [unique_smiles, todo_unique_smiles])
        unique_scores = np.concatenate(
            [unique_scores, todo_unique_scores])

    # get score for this batch
    todo_index = [np.where(unique_smiles == s)[
        0][0] for s in todo_smiles]
...
def sim_function(
    ...
    sim = np.asarray(
        tanimoto_sim(smiles, init_smiles),
        dtype=np.float32,
    )
    return sim
...
def has_substructure(smiles, sub_smiles=None):
    ...
    has_substr = [m.HasSubstructMatch(
        sub_m) for m in search_ms]
    return has_substr
...
def decoder_function(
    latent_tokens, cached_smiles, inferencer, reverse_alphabet, beam_size=4
):
    """
    For decoder g(S|z).
    """
    ...
    # (dbs*bm,) -> (dbs, bm)
    output_smiles = [tokens2smiles(
        t, reverse_alphabet) for t in output_tokens]
    ...
    sanitized_output_graph_features = [
        ...
        for smi in sanitized_output_smiles
    ]
        ...
        top_key: {
            sub_key: np.stack(
                [
                    ...
                    for this_graph in sanitized_output_graph_features
                ]
            )
    ...
    return {"graphs": batched_output_graph_features, "smiles": sanitized_output_smiles}
...
def fast_non_dominated_sorting(scores, constraints, constraint_weights=None):
    ...
    # (N, C) -> (N,) -> (N, N)
    and_constraints_i = np.all(
        constraints, axis=1).astype(np.int32)
    and_constraints_ij = and_constraints_i[:,
                                           None] + and_constraints_i[None, :]
    ...
    if constraint_weights is None:
        constraint_weights = np.ones(c_)
    weighted_constraints_i = constraints @ constraint_weights
    _compare_i_dom_j = (
        weighted_constraints_i[:,
                               None] > weighted_constraints_i[None, :]
    ).astype(np.int32)
    _compare_j_dom_i = (
        weighted_constraints_i[:,
                               None] < weighted_constraints_i[None, :]
    ...
    _compare_i_dom_j = (and_constraints_i[:, None] > and_constraints_i[None, :]).astype(
        np.int32
    )
    _compare_j_dom_i = (and_constraints_i[:, None] < and_constraints_i[None, :]).astype(
        np.int32
    )
    ...
    # (N, 1, M), (1, N, M) -> (N, N, M)
    _compare_i_dom_j = (scores[:, None] > scores[None, :]).astype(
        np.int32).prod(axis=2)
    _compare_j_dom_i = (scores[:, None] < scores[None, :]).astype(
        np.int32).prod(axis=2)
    dominated_ij_2 = _compare_i_dom_j - _compare_j_dom_i

    dominated_ij = (
        dominated_ij_0 *
        (and_constraints_ij == 0).astype(np.int32)
        + dominated_ij_1 *
        (and_constraints_ij == 1).astype(np.int32)
        + dominated_ij_2 *
        (and_constraints_ij == 2).astype(np.int32)
    )

    # (N, N) -> (N,)
    np_i = np.sum(dominated_ij == -1, axis=1)
    # [(n_i,), ...]
    sp_i = [np.where(dominated_ij[_] == 1)[0]
            for _ in range(n_)]
...
def crowding_distance_assignment(scores_it, inf=1e6):
    ...
    for _i in range(m_):
        ...
        # for i = 2 to Ni - 1
        # (Ni,)
        sorted_scores = front_scores_it[sorted_index]
        sorted_scores_ip1 = sorted_scores[2:]
        sorted_scores_im1 = sorted_scores[:-2]
        normed_distance_ = (sorted_scores_ip1 - sorted_scores_im1) / (
            sorted_scores[-1] - sorted_scores[0] + 1e-12
        )
        distance[sorted_index[1:-1]
                 ] += normed_distance_  # (Ni - 2,)
...
def NSGA_II(scores, constraints, constraint_weights=None, n_pops=128, inf=1e6):
    ...
    pareto_fronts = fast_non_dominated_sorting(
        scores, constraints, constraint_weights)
    ...
    while next_pop_ids.size + pareto_fronts[rank].size <= n_pops:
        next_pop_ids = np.concatenate(
            [next_pop_ids, pareto_fronts[rank]])
        rank += 1
    distance = crowding_distance_assignment(
        scores[pareto_fronts[rank]], inf)
    next_pop_ids = np.concatenate(
        [
            next_pop_ids,
            pareto_fronts[rank][
                np.argsort(distance)[
                    ::-1][: n_pops - next_pop_ids.size]
            ],
        ]
    )
    assert (
        next_pop_ids.size == n_pops
