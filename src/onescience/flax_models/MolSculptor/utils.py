...
def dual_inhibitor_reward_function(
    ...
    if todo_unique_smiles.size > 0:
        ...
        r_dock_1 = np.asarray(r_dock_1, np.float32) * (-1.0)  # (N,)
        ...
        r_dock_2 = np.asarray(r_dock_2, np.float32) * (-1.0)  # (N,)
...
def decoder_function(
    ...
    latent_tokens = safe_l2_normalize(latent_tokens / np.sqrt(d), axis=-1)
    ## (dbs*bm, n_seq)
...
def fast_non_dominated_sorting(scores, constraints, constraint_weights=None):
    ...
    _compare_i_dom_j = (
        weighted_constraints_i[:, None] > weighted_constraints_i[None, :]
    ).astype(np.int32)
    _compare_j_dom_i = (
        weighted_constraints_i[:, None] < weighted_constraints_i[None, :]
    ).astype(np.int32)
    ...
    dominated_ij = (
        dominated_ij_0 * (and_constraints_ij == 0).astype(np.int32)
...
def NSGA_II(scores, constraints, constraint_weights=None, n_pops=128, inf=1e6):
    ...
    next_pop_ids = np.concatenate(
        [
            next_pop_ids,
            pareto_fronts[rank][
                np.argsort(distance)[::-1][: n_pops - next_pop_ids.size]
            ],
        ]
    )
    assert (
