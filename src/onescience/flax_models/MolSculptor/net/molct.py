    ...
    def __call__(self, atom_emb, atom_mask, bond_emb, bond_mask, neighbor_list=None):
        ...
        def _update(atom_feature, acc_atom_feature, bond_feature, acc_bond_feature):
            ...
            if sparse_flag:
                ...
                neighbor_atom_feature = jnp.expand_dims(
                    atom_feature, axis=-3)
                neighbor_atom_feature = jnp.tile(
                    neighbor_atom_feature, (1,
                                            atom_feature.shape[-2], 1, 1)
                )
        ...
        acc_atom_feat = NormBlock(
            norm_method, norm_small)(acc_atom_feat)
        acc_bond_feat = NormBlock(
            norm_method, norm_small)(acc_bond_feat)
