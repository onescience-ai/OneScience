...
class EmbeddingBlock(nn.Module):
    ...
    def __call__(
        ...
        atom_type_emb = nn.Embed(
            num_embeddings=atom_emb_config.n_atom_type,
            features=dim_atom_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(atom_type)
        formal_charge_emb = nn.Embed(
            num_embeddings=atom_emb_config.n_formal_charge,
            features=dim_atom_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(formal_charge + 1)
        num_H_emb = nn.Embed(
            num_embeddings=atom_emb_config.n_num_H,
            features=dim_atom_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(num_H)
        aromaticity_emb = nn.Embed(
            num_embeddings=atom_emb_config.n_aromaticity,
            features=dim_atom_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(aromaticity)
        hybridization_emb = nn.Embed(
            num_embeddings=atom_emb_config.n_hybridization,
            features=dim_atom_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(hybridization)
        chiral_emb = nn.Embed(
            num_embeddings=atom_emb_config.n_chiral,
            features=dim_atom_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(chiral)
        ...
        bond_type_emb = nn.Embed(
            num_embeddings=bond_emb_config.n_bond_type,
            features=dim_bond_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(bond_type)
        stereo_emb = nn.Embed(
            num_embeddings=bond_emb_config.n_stereo,
            features=dim_bond_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(stereo)
        conjugated_emb = nn.Embed(
            num_embeddings=bond_emb_config.n_conjugated,
            features=dim_bond_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(conjugated)
        in_ring_emb = nn.Embed(
            num_embeddings=bond_emb_config.n_in_ring,
            features=dim_bond_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(in_ring)
        graph_distance_emb = nn.Embed(
            num_embeddings=bond_emb_config.n_graph_distance,
            features=dim_bond_feature,
            dtype=_dtype,
            param_dtype=jnp.float32,
        )(graph_distance)
