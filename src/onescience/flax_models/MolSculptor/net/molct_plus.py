...
class MOLCT_PLUS(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, atom_raw_feat, atom_mask, pair_raw_feat, pair_mask, neighbor_list=None
    ):
        r"""MolCT+ by ZhangJ.
        Args:
            atom_raw_feat: [B, A, C];
        ...
        acc_atom_feat = NormBlock(
            norm_method, norm_eps)(acc_atom_feat)
        acc_pair_feat = NormBlock(
            norm_method, norm_eps)(acc_pair_feat)
...
class FeatureTransformer(nn.Module):
    ...
    def __call__(self, atom_raw_feat, pair_raw_feat):
        ...
        # (..., A, A, C') -> (..., A, A, F'):
        pair_act = nn.Dense(
            features=pair_act_dim,
            dtype=_dtype,
            param_dtype=jnp.float32,
            kernel_init=lecun_normal(),
            name="pair_activations",
        )(pair_raw_feat)
        # (..., A, C) -> (..., A, F'):
        left_act = nn.Dense(
            features=pair_act_dim,
        ...
        # (..., A, 1, F) + (..., 1, A, F) -> (..., A, A, F)
        pair_act += jnp.expand_dims(left_act, -2) + \
            jnp.expand_dims(right_act, -3)
...
class PairFeatureUpdate(nn.Module):
    ...
    def __call__(self, s_i, s_j, m_i, m_j, z_ij, acc_z_ij, m_ij):
        ...
        act, d_act = act, act
        d_act = Transition(
            self.config.transition, self.global_config)(d_act)
        d_act = nn.Dropout(
            rate=self.config.dropout_rate, deterministic=(
                not dropout_flag)
        )(d_act)
...
class InteractionUnit(nn.Module):
    ...
    def __call__(
        ...
        def _atom_update(
            atom_feat, acc_atom_feat, atom_mask, pair_feat, pair_mask, neighbor_list
        ):
            ...
            if sparse_flag:
                ...
                neighbor_mask = gather_neighbor(
                    atom_mask[...,
                              None], neighbor_list, is_pair=False
                )
                neighbor_mask = jnp.squeeze(
                    neighbor_mask, axis=-1)
            else:
                # (B, A) -> (B, 1, A)
                neighbor_mask = jnp.expand_dims(
                    atom_mask, axis=-2)

            # 2. atom feature update
            atom_feat, acc_atom_feat = ResiDualTransformerBlock(
                self.config.node_update,
                self.global_config,
            )(
                s_i=atom_feat,
                acc_s_i=acc_atom_feat,
                m_i=atom_mask,
                m_j=neighbor_mask,
                z_ij=pair_feat,
                m_ij=pair_mask,
        ...
        def _pair_update(
            ...
            if sparse_flag:
                # (B, A, Fa) -> (B, A, A', Fa)
                neighbor_atom_feat = gather_neighbor(
                    atom_feat, neighbor_list, is_pair=True
                )
            else:
                # (B, A, Fa) -> (B, 1, A, Fa) -> (B, A, A, Fa)
                neighbor_atom_feat = jnp.expand_dims(
                    atom_feat, axis=-3)
                neighbor_atom_feat = jnp.tile(
                    neighbor_atom_feat, (1,
                                         atom_feat.shape[-2], 1, 1)
                )
...
class PrefixGraphTransformerPLUS(nn.Module):
    ...
    def create_raw_feature(self, atom_features, bond_features):
        ...
        formal_charge = jax.nn.one_hot(
            ...
            num_classes=atom_emb_config.n_formal_charge,
            dtype=arr_dtype,
        )
        num_H = jax.nn.one_hot(
            atom_features["num_H"],
            num_classes=atom_emb_config.n_num_H,
            dtype=arr_dtype,
        )
        aromaticity = jax.nn.one_hot(
            atom_features["aromaticity"],
            num_classes=atom_emb_config.n_aromaticity,
            dtype=arr_dtype,
        )
        hybridization = jax.nn.one_hot(
            atom_features["hybridization"],
            num_classes=atom_emb_config.n_hybridization,
            dtype=arr_dtype,
        )
        chirality = jax.nn.one_hot(
            atom_features["chiral"],
        ...
        prefix_raw_feat = jnp.repeat(
            prefix_raw_feat[None, ...],
            repeats=bs,
            axis=0,
        )  # (B, num_prefix, C + num_prefix)
        atom_raw_feat = jnp.concatenate(
            [prefix_raw_feat, atom_raw_feat],
            axis=1,
        )
        atom_mask = jnp.pad(
            atom_features["atom_mask"],
            ((0, 0), (n_p, 0)),
            mode="constant",
            constant_values=1,
        )
        ...
        bond_type = jax.nn.one_hot(
            ...
            num_classes=bond_emb_config.n_bond_type,
            dtype=arr_dtype,
        )
        stereo = jax.nn.one_hot(
            bond_features["stereo"],
            num_classes=bond_emb_config.n_stereo,
            dtype=arr_dtype,
        )
        conjugated = jax.nn.one_hot(
            bond_features["conjugated"],
            num_classes=bond_emb_config.n_conjugated,
            dtype=arr_dtype,
        )
        in_ring = jax.nn.one_hot(
            bond_features["in_ring"],
            num_classes=bond_emb_config.n_in_ring,
            dtype=arr_dtype,
        )
        ...
        bond_raw_feat = jnp.where(
            prefix_bond_mask[..., None],
            prefix_bond_raw_feat,
            bond_raw_feat,
        )  # (B, A + num_prefix, A + num_prefix, C' + 1)
        bond_mask = jnp.pad(
            bond_features["bond_mask"],
            ((0, 0), (n_p, 0), (n_p, 0)),
            mode="constant",
            constant_values=1,
        )
