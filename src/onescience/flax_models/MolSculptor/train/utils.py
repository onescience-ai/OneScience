...
def psum_tree(values):
    return jax.tree_util.tree_map(lambda x: jax.lax.psum(x, axis_name="i"), values)


@jax.jit
def pmean_tree(values):
    return jax.tree_util.tree_map(lambda x: jax.lax.pmean(x, axis_name="i"), values)
