...
class GaussianDiffusion:
    def __init__(self, config):
        ...
        beta_end = scale * 0.02
        betas = jnp.linspace(
            beta_start, beta_end, num_diffusion_timesteps, dtype=jnp.float32
        )
        self.betas = betas
        ...
        self.posterior_mean_coef1 = (
            betas * jnp.sqrt(self.alphas_cumprod_prev) /
            (1.0 - self.alphas_cumprod)
        )
    ...
    def alphas_cumprod_to_t(self, alphas_cumprod_val):
        ...
        index = jnp.searchsorted(
            self.alphas_cumprod[::-1], alphas_cumprod_val)
        ...
        return (
            ...
            - 1
            + (alphas_cumprod_val -
               self.alphas_cumprod[index - 1])
            / (self.alphas_cumprod[index] - self.alphas_cumprod[index - 1])
        )
    ...
    # q(x_t | x_{t-1})
    def q_sample_step(self, x, t, noise):
        """q(x_t | x_{t-1}) = N(x_t | sqrt(1 - beta_t) * x_{t-1}, beta_t I)))"""
        return (
            _extract_into_tensor(
                self.sqrt_alphas, t, x.shape) * x
    ...
    def p_mean_variance(self, x_t, t, eps, clamp_x0_fn=None, clip=True):
        ...
        pred_xstart = jnp.where(
            clip, pred_xstart_clipped, pred_xstart)
        if clamp_x0_fn:
            pred_xstart = clamp_x0_fn(pred_xstart)

        model_mean = (
            _extract_into_tensor(
                self.posterior_mean_coef1, t, x_t.shape) * pred_xstart
            + _extract_into_tensor(self.posterior_mean_coef2, t, x_t.shape) * x_t
        )
        return model_mean, model_variance, model_log_variance

    def p_ddim(self, x_t, t, eps):
        at = _extract_into_tensor(
