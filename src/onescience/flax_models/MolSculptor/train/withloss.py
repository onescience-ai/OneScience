...
def gaussian_kernel_mmd(x, y, sigma):
    ...
    def _gaussian_kernel(x, y, sigma=sigma):
        ...
        y = y[None, :]
        # (B, B)
        return jnp.exp((-1) * jnp.sum((x - y) ** 2, axis=-1) / (sigma**2))
...
def info_nce_function(f, pair_f, candidate_fs, mask, beta=1.0):
...
def align_loss_function(x, y, alpha=2.0, beta=1.0):
    # x / y: (b, Q, D)
    ...
    # (b,) ### mean mode deprecated
    dist_ = jnp.mean(dist_, axis=1)
    # dist_ = jnp.max(dist_, axis = 1) ## (b,)
    return jnp.mean(dist_, axis=0)  # (,)


def uni_loss_function(x, t=2.0, beta=1.0):
    # x: (b, Q, D)
...
# simCLR
def align_loss_function_sr(x, y, alpha=2.0):
    # x / y: (b, f)
    b, f = x.shape
...
def uni_loss_function_sr(x, t=2.0):
    # x: (b, f)
    # breakpoint() ## check here
    b, qd = x.shape
...
class L2SeqGenWithLoss(nn.Module):
    ...
    def setup(self):
        ...
        self.vocab_size = (
            settings_config.vocab_size
        )  # vocab size include BOS/EOS/UNK/SEP...
    ...
    def __call__(self, input_features, labels, step_it=0):
        ...
        seq_logits, seq_mask, seq_label = jax.tree_util.tree_map(
            jnp.float32, (seq_logits,
                          labels["mask"], labels["label"])
        )
        # reshape
        batch_size, num_k, n_seq = seq_mask.shape
        seq_mask, seq_label = jax.tree_util.tree_map(
            lambda f: f.reshape(
                batch_size * num_k, n_seq), (seq_mask, seq_label)
        )  # (BK, N), (BK, N)
        ...
        log_p_unmasked = (-1) * softmax_cross_entropy(
            seq_logits, seq_label, seq_mask
        )  # (BK,)
        log_p_unmasked = log_p_unmasked.reshape(
            batch_size, num_k)  # (B, K)
        mask_p = jnp.reshape(
            seq_mask.sum(-1) > 0, (batch_size, num_k))
        k = jnp.sum(mask_p, -1)  # (B,)
        recon_loss = jnp.sum(
            log_p_unmasked, -1) / (k * (n_seq - n_prefix))  # (B,)
        ...
        loss = (
            recon_loss * self.loss_weights["reconstruct"]
...
class DecoderWithLoss(nn.Module):
    ...
    def __call__(self, input_features, labels, step_it=0):
        ...
        seq_logits = self.generator(
            input_features["graph_feat"], input_features["seq_feat"]
        )
        # cast to fp32
        seq_logits, seq_mask, seq_label = jax.tree_util.tree_map(
            jnp.float32, (seq_logits,
...
class MMDSeqGenWithLoss(nn.Module):
    ...
    def __call__(self, input_features, labels, step_it=0):
        ...
        latent_feat = jnp.float32(latent_feat)
        latent_feat_x, latent_feat_y = jnp.split(
            latent_feat, 2, axis=0
        )  # (B, Q, D) -> (B/2, Q, D)
        # gather from processes
        # WARNING: in this code all processes should have the same sample distribution: b/2 for cluster center and b/2 for neighbor
        # NO MASK HERE
        if self.pmap_flag:
            latent_feat_x = jax.lax.all_gather(
                latent_feat_x, axis_name="i", tiled=True
            )  # (B/2*N, Q, D), N = n_global_devices
            latent_feat_y = jax.lax.all_gather(
                latent_feat_y, axis_name="i", tiled=True)
        # breakpoint() ## check shape here
        cts_config = self.train_config.contrastive
        align_loss = align_loss_function(
            latent_feat_x, latent_feat_y, alpha=cts_config.alpha
        )  # (,)
        uni_loss = 0.5 * (
            uni_loss_function(latent_feat_x, t=cts_config.t)
            + uni_loss_function(latent_feat_y,
                                t=cts_config.t)
        )  # (,)

        ######### get mmd regularization loss #########
        mmd_loss = 0.0
        latent_feat = aux["graph_feat"].reshape(
            batch_size, -1)  # (B, Q, D) -> (B, QD)
        latent_feat = jnp.float32(latent_feat)
        gaussian_feat = jax.random.normal(
            key=self.make_rng("latent"),
            shape=latent_feat.shape,
            dtype=latent_feat.dtype,
        )
        for _sigma in self.train_config.mmd_sigma:
            mmd_loss += gaussian_kernel_mmd(
                latent_feat, gaussian_feat, _sigma)

        loss = (
            recon_loss * self.loss_weights["reconstruct"]
            + mmd_loss * self.loss_weights["mmd"]
            + align_loss * self.loss_weights["align"]
            + uni_loss * self.loss_weights["uniformity"]
        )  # the lambda in paper
        loss_dict = {
            "loss": loss,
            "reconstruct": recon_loss,
...
class DiTWithLoss(nn.Module):
    ...
    def __call__(self, input_dict):
        ...
        random_t = jax.random.randint(
            time_key, (bs,), 0, time_steps)
        normal_key = self.make_rng("normal_key")
        eps = jax.random.normal(
            normal_key, x_0.shape, dtype=jnp.float32)
        x_t = self.scheduler.q_sample(x_0, random_t, eps)

        # run
        x_t, random_t = jtu.tree_map(
            lambda arr: arr.astype(arr_dtype),
            (x_t, random_t),
        )
        eps_pred = self.net(
            x_t,
            jnp.ones((bs, npt), dtype=arr_dtype),
            random_t,
            tokens_rope_index=jnp.arange(npt, dtype=jnp.int32)[None, :].repeat(
                bs, axis=0
            ),
        )
