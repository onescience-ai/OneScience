...
def make_graph_feature(
    graph_feature, n_padding_atom: int = 32, dtype: np.dtype = np.int16
):
...
def make_sequence_feature(
    ...
    def pad_or_crop_k(
        ...
        if n_sf <= k:  # If n_sf <= k, just padding
            ...
            input_mask = np.pad(
                input_mask, ((0, n_pad), (0, 0)), mode="constant", constant_values=0
            )
            label = np.pad(
                label, ((0, n_pad), (0, 0)), mode="constant", constant_values=0
            )
            label_mask = np.pad(
...
def make_feature(
    ...
    graph_dict = make_graph_feature(
        graph_feature, data_config["n_pad_atom"], data_config["dtype"]
    )
    seq_dict = make_sequence_feature(
        seq_tokens,
        data_config,
    )
...
class AEDataLoader:
    ...
    def organize(self, step_it):
        ...
        def _get_idxs(
            ...
            start_it = (
                ...
                + self.local_batch_size * self.load_int * self.process_id
            )
            stop_it = (
                step_it * self.global_batch_size
    ...
    def _make_feature(self, name_list_trunk, data_config):
        ...
        graph_feat, sequence_feat = make_feature(
            data_from_pickle,
            data_config,
        )
        input_feat = {
            ...
            "sequence_features": {
                "tokens": np.array(sequence_feat["input_tokens"], data_config["dtype"]),
