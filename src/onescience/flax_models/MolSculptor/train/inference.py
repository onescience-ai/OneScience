...
def greedy_search(logits, previous_tokens, step_it):
    ...
    # (batch_size, n_seq - n_q)
    pred_token = jnp.argmax(logits, -1)
    output_tokens = previous_tokens.at[:,
                                       step_it + 1].set(pred_token[:, step_it])
...
def beam_search(
    ...
    # (bs, bm, n, n_token)
    logits = logits.reshape(-1, beam_size, n, n_token)
    logits = logits[:, :, step_it, :]  # (bs, bm, n_token)
    log_prob_last_token = jax.nn.log_softmax(
        logits, -1)  # (bs, bm, n_token)
    ...
    log_prob_last_token_topk = jnp.sort(
        log_prob_last_token, -1)[:, :, -beam_size:]
    last_token_topk = jnp.argsort(log_prob_last_token, -1)[
        :, :, -beam_size:
    ]  # (bs, bm, bm)
    # (..., beam_size, 1) + (..., beam_size, beam_size)
    # Log(p1*p2*...pt-1) + Log(pt)
    log_prob_accum = previous_log_probs[...,
                                        None] + log_prob_last_token_topk
    # (bs, bm * bm)
    log_prob_accum = log_prob_accum.reshape(
        -1, beam_size * beam_size)
    log_prob_accum += (1.0 - search_mask[None, :]) * (-1e5)
    # (bs * bm, 1, n)
    tokens_accum = previous_tokens[:, None, :]
    # (batch_size*beam_size, beam_size, n)
    tokens_accum = jnp.tile(tokens_accum, (1, beam_size, 1)).reshape(
        -1, beam_size, beam_size, n
    )  # (bs, bm, bm, n)
    tokens_accum = tokens_accum.at[:, :,
                                   :, step_it + 1].set(last_token_topk)
    tokens_accum = tokens_accum.reshape(
        -1, beam_size * beam_size, n
    )  # (bs, bm * bm, n)
    top_k_sequence_id = jnp.argsort(
        log_prob_accum, -1)[:, -beam_size:]  # (bs, bm)
    # (batch_size, beam_size*beam_size, n) -> (batch_size, beam_size, n)
    output_tokens = jax.vmap(_take, in_axes=(0, 0))(
        tokens_accum, top_k_sequence_id)
    # (batch_size, beam_size*beam_size) -> (batch_size, beam_size)
    output_log_prob_accum = jax.vmap(_take, in_axes=(0, 0))(
        log_prob_accum, top_k_sequence_id
    )
...
def top_p_search(logits, p, rng_key, previous_tokens, step_it):
    ...
    sorted_probs = jax.nn.softmax(
        logits, -1)  # (bs, n_token)
    cum_probs = jnp.cumsum(
        sorted_probs, -1)  # (bs, n_token)
    mask = cum_probs > p
    ...
    previous_tokens = previous_tokens.at[:,
                                         step_it + 1].set(selected_token_ids)
...
def top_k_search(logits, k, rng_key, previous_tokens, step_it):
    ...
    bs, n, n_token = logits.shape
    logits = logits[:, step_it, :]  # (bs, n_token)
    top_k_indices = jnp.argsort(
        logits, -1)[:, -k:]  # (bs, k)
    top_k_logits = logits[top_k_indices]
    top_k_log_probs = jax.nn.log_softmax(
        top_k_logits, -1)  # (bs, k)
...
class InferEncoder(nn.Module):
    ...
    def __call__(
        ...
        graph_feat = nn.Dense(
            ...
            kernel_init=truncated_normal(0.01),
            dtype=arr_dtype,
            use_bias=False,
...
class InferDecoder(nn.Module):
    ...
    def __call__(self, sequence_features, graph_feat):
        ...
        seq_rope_index = jnp.arange(
            num_tokens, dtype=jnp.int32)[None, :]
...
class Inferencer:
    ...
    def __init__(
        ...
        assert self.sampling_method in [
            "greedy", "beam", "top_p", "top_k", "nucleus"]

        self.encoding_function = jax.jit(encoding_net.apply)
        self.decoding_function = jax.jit(decoding_net.apply)

        self.jit_sample_per_step = jax.jit(
            self._sample_per_step)
        self.jit_encoding_graphs = jax.jit(
            self.encoding_graphs)

        self.pmap_sample_per_step = jax.pmap(
            self._sample_per_step, axis_name="i")
        self.pmap_encoding_graphs = jax.pmap(
            self.encoding_graphs, axis_name="i")
    ...
    def _sample_per_step(self, input_tokens_it, step_it, mask_it, cached_arr):
        ...
        output_logits = self.decoding_function(
            ...
            {"tokens": input_tokens_it, "mask": mask_it},
            cond,
        )  # (batch_size, n_seq, vocab_size)
        output_logits = output_logits[
            :, num_prefix_tokens:, :
        ]  # (batch_size, n, n_tokens)

        # sampling
        if self.sampling_method == "greedy":
            output_tokens = greedy_search(
                output_logits, input_tokens_it, step_it)

        elif self.sampling_method == "beam":
            beam_size = self.config.beam_size
            output_tokens, output_log_prob_sum = beam_search(
                output_logits,
                cached_arr["log_prob_sum"],
                input_tokens_it,
                step_it,
                cached_arr["search_mask"],
                beam_size,
            )
            ...
            raise NotImplementedError(
                f"Sampling method {self.sampling_method} is not implemented"
            )

        output_tokens = jnp.pad(
            output_tokens,
            ((0, 0), (0, n_seq - output_tokens.shape[-1])),
            mode="constant",
            constant_values=1,
        )  # pad eos
    ...
    def encoding_graphs(self, graph_features):
        ...
        return self.encoding_function(
            {"params": self.encoding_params}, atom_features, bond_features
        )
    ...
    def beam_search(self, step, cond, input_tokens=None):
        ...
        if input_tokens is None:
            ...
            input_tokens = np.full(
                (batch_size * beam_size, 1), self.config.bos_token, dtype=np.int32
            )
            input_tokens = np.concatenate(
                [
                    input_tokens,
                    np.full(
                        (batch_size * beam_size, n_seq - 1),
        ...
        # make cond feat
        cached_arr_["log_prob_sum"] = np.zeros(
            (batch_size, beam_size))
        _shape = (-1,) + cond.shape[1:]
        cond = np.tile(cond[:, None], (1, beam_size, 1, 1)).reshape(
            _shape
        )  # (bs*bm, n_prefix, dim_feat)
        ...
        for step_it in range(step, step_limit):
            ...
            if step_it == 0:  # first step for beam search
                cached_arr_["search_mask"] = np.pad(
                    np.ones((beam_size,), np.int32),
                    (0, beam_size * beam_size - beam_size),
                    mode="constant",
                    constant_values=0,
                )

            mask = np.ones(
                (batch_size * beam_size, step_it + 1))
            mask = np.pad(
                mask,
                ((0, 0), (0, n_seq - step_it - 1)),
                mode="constant",
                constant_values=0,
            )

            # jit per step
            input_tokens, cached_arr_ = self.jit_sample_per_step(
                input_tokens, step_it, mask, cached_arr_
            )

            if step_it == 0:
                cached_arr_["search_mask"] = np.ones(
                    (beam_size * beam_size,), np.int32)

            # eos check
            # (bs*bm,)
            this_tokens = input_tokens[:, step_it + 1]
    ...
    def beam_search_multi_device(self, step, cond, input_tokens=None):
        ...
        if input_tokens is None:
            ...
            input_tokens = np.full(
                (dbs * beam_size, 1), self.config.bos_token, dtype=np.int32
            )
            input_tokens = np.concatenate(
                [
                    input_tokens,
                    np.full(
                        (dbs * beam_size, nseq - 1),
                        self.config.eos_token,
                        dtype=np.int32,
                    ),
                ],
        ...
        cached_arr_["log_prob_sum"] = replicate(
            log_prob_sum_)
        ...
        for step_it in range(step, step_limit):
            ...
            if step_it == 0:  # first step for beam search
                ...
                cached_arr_["search_mask"] = replicate(
                    search_mask_
                )  # (n_device, beam_size*beam_size,)

            # (n_device, bs*bm, n_seq)
            mask = np.ones((dbs * beam_size, step_it + 1))
            mask = np.pad(
                mask,
                ((0, 0), (0, nseq - step_it - 1)),
                mode="constant",
                constant_values=0,
            )
            mask = replicate(mask)
            ...
            input_tokens, cached_arr_ = self.pmap_sample_per_step(
                input_tokens, input_step_it, mask, cached_arr_
            )

            if step_it == 0:
                search_mask_ = np.ones(
                    (beam_size * beam_size,), np.int32)
                cached_arr_["search_mask"] = replicate(
                    search_mask_)
            ...
            this_tokens = input_tokens[:, step_it + 1]
            stop_flag = (
                this_tokens == self.config.eos_token).all()
            if stop_flag:
                break
...
def standardize(smiles, dropout_threshold=0.7):
    ...
    try:
        with BlockLogs():
            ...
            cs = Chem.MolToSmiles(
                m_, isomericSmiles=True, canonical=True)
        return cs
    except:
        return None
...
def encoding_graphs(smi):
    ...
    Chem.SanitizeMol(
        mol_to_process, Chem.SANITIZE_ALL ^ Chem.SANITIZE_PROPERTIES)
    mol_to_process.UpdatePropertyCache(strict=False)
    Chem.AssignStereochemistry(
        mol_to_process, cleanIt=True, force=True)
    ...
    for atom in mol_to_process.GetAtoms():
        if atom.HasProp("_CIPCode"):
            try:
                chiral.append(
                    chiral_f[atom.GetProp("_CIPCode")])
            except:
                chiral.append(3)  # unk
        else:
    ...
    bond_type = np.zeros(
        (num_atoms, num_atoms), ENCODING_DTYPE)
    for bond in mol_to_process.GetBonds():
        bond_type[bond.GetBeginAtomIdx(
        ), bond.GetEndAtomIdx()] = bond.GetBondType()
        bond_type[bond.GetEndAtomIdx(
        ), bond.GetBeginAtomIdx()] = bond.GetBondType()
    stereo = np.zeros(
        (num_atoms, num_atoms), ENCODING_DTYPE)
    for bond in mol_to_process.GetBonds():
        stereo[bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()
               ] = bond.GetStereo()
        stereo[bond.GetEndAtomIdx(), bond.GetBeginAtomIdx()
               ] = bond.GetStereo()
    conjugated = np.zeros(
        (num_atoms, num_atoms), ENCODING_DTYPE)
    for bond in mol_to_process.GetBonds():
        conjugated[bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()] = (
            bond.GetIsConjugated() + 1
        )
        conjugated[bond.GetEndAtomIdx(), bond.GetBeginAtomIdx()] = (
            bond.GetIsConjugated() + 1
        )
    in_ring = np.zeros(
        (num_atoms, num_atoms), ENCODING_DTYPE)
    for bond in mol_to_process.GetBonds():
        in_ring[bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()
                ] = bond.IsInRing() + 1
        in_ring[bond.GetEndAtomIdx(), bond.GetBeginAtomIdx()] = (
            bond.IsInRing() + 1
        )  # for unk
    graph_distance = np.zeros(
        (num_atoms, num_atoms), ENCODING_DTYPE
    )  # 24-06-19 add graph distance
    distance_matrix = rdmolops.GetDistanceMatrix(
        mol_to_process)
    graph_distance = np.asarray(
        distance_matrix, dtype=ENCODING_DTYPE)
...
def padding_graph(
    arr: np.ndarray, padding_num: int = 64, dtype: np.dtype = ENCODING_DTYPE
):
    ...
    if arr.ndim < 2:
        ...
        raise ValueError(
            "The ndim of input array should be less than 2.")


def make_graph_feature(
    graph_feature,
    n_padding_atom: int = 64,
):
...
def smi2graph_features(smi, n_padding_atom: int = 64):
    ...
    graph_feature = make_graph_feature(
        graph_feature, n_padding_atom=n_padding_atom)
    return graph_feature
