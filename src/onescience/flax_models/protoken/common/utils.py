...
def dgram_from_positions(positions, num_bins, min_bin, max_bin, ret_type):
    ...
    dgram = (dist2 > lower_breaks).astype(ret_type) * (dist2 < upper_breaks).astype(
        ret_type
    )
    return dgram
...
def batch_torsion_angles_to_frames(
    aatype, backb_to_global, torsion_angles_sin_cos, restype_rigid_group_default_frame
):
    ...
    sin_angles = jnp.concatenate(
        [jnp.zeros([num_batch, num_residues, 1]), sin_angles], axis=-1
    )
    ...
    chi3_frame_to_backb = geometry.rigids_mul_rigids(
        chi2_frame_to_backb, chi3_frame_to_frame
    )
    chi4_frame_to_backb = geometry.rigids_mul_rigids(
        chi3_frame_to_backb, chi4_frame_to_frame
    )

    # Recombine them to a Rigids with shape (N, 8).
    all_frames_to_backb = batch_rigids_concate_all(
        all_frames, chi2_frame_to_backb, chi3_frame_to_backb, chi4_frame_to_backb
    )

    backb_to_global = (
        geometry.rots_expand_dims(backb_to_global[0], -1),
        geometry.vecs_expand_dims(backb_to_global[1], -1),
    )
    # Create the global frames.
    all_frames_to_global = geometry.rigids_mul_rigids(
        backb_to_global, all_frames_to_backb
    )
...
def batch_frames_and_literature_positions_to_atom14_pos(
    aatype,
...
def torsion_angles_to_frames(
    ...
    # Recombine them to a Rigids with shape (N, 8).
    all_frames_to_backb = rigids_concate_all(
        all_frames, chi2_frame_to_backb, chi3_frame_to_backb, chi4_frame_to_backb
    )
