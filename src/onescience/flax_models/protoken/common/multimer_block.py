...
def multimer_rigids_from_3_points(vec_a, vec_b, vec_c):
    ...
    m = multimer_rots_from_two_vecs(
        e0_unnormalized=vecs_sub(vec_c, vec_b), e1_unnormalized=vecs_sub(vec_a, vec_b)
    )
    rigid = (m, vec_b)
    return rigid
...
class MultimerInvariantPointAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.projection_num = (
            self.num_head * self.num_scalar_v
            + self.num_head * self.num_point_v * 4
            + self.num_head * self.pair_dim
        )
        self.q_scalar = nn.Dense(
            features=self.num_head * self.num_scalar_qk,
            kernel_init=lecun_normal(),
            has_bias=False,
        )
        self.k_scalar = nn.Dense(
            features=self.num_head * self.num_scalar_qk,
            kernel_init=lecun_normal(),
            has_bias=False,
        )
        self.v_scalar = nn.Dense(
            features=self.num_head * self.num_scalar_v,
            kernel_init=lecun_normal(),
            has_bias=False,
        )
        self.q_point_local = nn.Dense(
            features=self.num_head * 3 * self.num_point_qk, kernel_init=lecun_normal()
        )
        self.k_point_local = nn.Dense(
            features=self.num_head * 3 * self.num_point_qk, kernel_init=lecun_normal()
        )
        self.v_point_local = nn.Dense(
            features=self.num_head * 3 * self.num_point_v, kernel_init=lecun_normal()
        )

        self.trainable_point_weights = self.param(
            name="trainable_point_weights",
            init_fn=ones_init(),
            shape=(12,),
            dtype=jnp.float32,
        )
    ...
    def __call__(self, inputs_1d, inputs_2d, mask, rotation, translation):
        ...
        q_point_local = (
            jnp.squeeze(q_point_local[0]),
            jnp.squeeze(q_point_local[1]),
        ...
        k_point_local = (
            jnp.squeeze(k_point_local[0]),
            jnp.squeeze(k_point_local[1]),
        ...
        v_point_local = (
            jnp.squeeze(v_point_local[0]),
            jnp.squeeze(v_point_local[1]),
        ...
        v_point = [
            ...
            v_point_global[2][None],
        ]

        result_point_global = [
            jnp.sum(attn[..., None] * v_point[0], axis=-3),
