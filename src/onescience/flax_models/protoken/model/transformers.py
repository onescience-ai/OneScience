...
class SelfResidualTransformer(nn.Module):
    ...
    pre_attention_operation_list: tuple = (
        "PairEmbedding",
    )  # ["PairEmbedding"] # ["PairEmbedding", "LN", "AttDropout"]
    post_attention_operation_list: tuple = (
        "Dropout",
        "LN",
    )  # ["Dropout", "LN"] # ["Dropout", "LN", "ResiDual"]
    post_ffn_operation_list: tuple = (
        "Dropout",
    )  # ["Dropout"] # ["Dropout", "LN", "ResiDual"]
    ...
    def __call__(
        ...
        q_act, k_act, v_act, pair_bias_fp32 = self.pre_attention(
        ...
        residual_act, accumulated_act = self.post_ffn(
            residual_act, ffn_output, q_mask, accumulated_act
        )
