...
from onescience.flax_models.protoken.modules.head import (
    DistogramHead,
    PredictedLDDTHead,
)
from onescience.flax_models.protoken.modules.structure import (
    FrameInitializer,
    StructureModule,
)
...
class VQ_Decoder(nn.Module):
    ...
    def setup(self):
        ...
        if self.pre_layer_norm:
            self.slv_layer_norm = ActFuncWrapper(
                nn.LayerNorm(
        ...
        self.vq_activations_left = nn.Dense(
            features=self.pair_channel,  # 256 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.vq_activations_right = nn.Dense(
            features=self.pair_channel,  # 256 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        ...
        self.pair_activations = nn.Dense(
            features=self.pair_channel,  # 64 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        ...
        self.module_distogram = DistogramHead(
            self.global_config, self.cfg.distogram, self.pair_channel
        )
    ...
    def __call__(self, vq_act, seq_mask, residue_index):
        ...
        mask_2d = jnp.expand_dims(seq_mask, axis=-1) * jnp.expand_dims(
            seq_mask, axis=-2
        )  # [num_batch, Nres, Nres]
        ...
        for i in range(self.pair_update_evoformer_stack_num):
            ...
            ) = self.pair_update_evoformer_stack[i](
        ...
        for it_evo in range(self.co_update_evoformer_stack_num):
            (
                ...
                accumulated_single_act,
                accumulated_pair_act,
            ) = self.co_update_evoformer_stack[it_evo](
                seq_act=single_act,  # 256
...
class Protein_Decoder(nn.Module):
    ...
    def setup(self):
        ...
        self.pre_sm_single_update = nn.Dense(
            self.esm_cfg.single_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )  # 256->384
        ...
        self.frame_initializer = FrameInitializer(
            global_config=self.global_config,
            config=self.fi_cfg,
        ...
        self.structure_decoder_module_lonely = StructureModule(
            global_config=self.global_config,
            config=self.esm_cfg,
            seq_length=self.seq_len,  # 256
            frozen_IPA=False,
            ...
            ret_single_pair=True,
        )  # 2023.11.02

        self.module_pLDDT = PredictedLDDTHead(
            global_config=self.global_config, cfg=self.cfg.predicted_lddt
        )
    ...
    def __call__(self, single_act, pair_act, seq_mask, aatype):
        ...
        (
            ...
            normed_single,
            normed_pair,
        ) = self.structure_decoder_module_lonely(
            single_activations,
            pair_activations,
            seq_mask,
            aatype,
        ...
        pLDDT_logits = self.module_pLDDT(
            single_activations, normed_pair, seq_mask, final_affines, aatype
        )
