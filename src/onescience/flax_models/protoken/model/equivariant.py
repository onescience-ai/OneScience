...
from onescience.flax_models.protoken.modules.basic import Softmax1
...
class InvariantPointAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.q_point_local = nn.Dense(
            features=self.num_head * 3 * self.num_point_qk,
            kernel_init=lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
    ...
    def __call__(self, inputs_1d, inputs_2d, mask, rotation, translation):
        ...
        point_weights = self.point_weights * jnp.expand_dims(
            trainable_point_weights, axis=1
        )

        v_point = [
            jnp.swapaxes(v_point0, -2, -3),
            jnp.swapaxes(v_point1, -2, -3),
            jnp.swapaxes(v_point2, -2, -3),
        ]
        q_point = [
            jnp.swapaxes(q_point0, -2, -3),
            jnp.swapaxes(q_point1, -2, -3),
            jnp.swapaxes(q_point2, -2, -3),
        ]
        k_point = [
            jnp.swapaxes(k_point0, -2, -3),
            jnp.swapaxes(k_point1, -2, -3),
            jnp.swapaxes(k_point2, -2, -3),
        ]
        ...
        attn_qk_point = -0.5 * jnp.sum(
            jnp.expand_dims(jnp.expand_dims(point_weights, 1), 1) * dist2, axis=-1
        )
