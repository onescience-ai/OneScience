...
from onescience.flax_models.protoken.modules.transformer_blocks import (
    OuterProduct,
    PostNonLinear,
    PreNonLinear,
    Transition,
...
class SeqBlock(nn.Module):
    ...
    pre_attention_operation_list: tuple = (
        "PairEmbedding",
        "LN",
    )  # = ["PairEmbedding", "LN"] # ["PairEmbedding", "PreLN", "AttDropout"]
    post_attention_operation_list: tuple = (
        "Dropout",
        "LN",
    )  # = ["Dropout", "LN"] # ["Dropout", "PostLN"]
    post_ffn_operation_list: tuple = (
        "Dropout",
    )  # = ["Dropout"] # ["Dropout", "PostLN", "ResidualLN"]
    init_method: str = "AF2"
    ...
    def setup(self):
        ...
        self.transformer = SelfResidualTransformer(
    ...
    def __call__(
        ...
        seq_act, accumulated_seq_act = self.transformer(
            seq_act, accumulated_seq_act, attention_masks, pair_act=pair_act
        )
...
class PairBlock(nn.Module):
    ...
    post_ffn_operation_list: tuple = (
        "Dropout",
    )  # = ["Dropout"] # ["PostLN", "Dropout", "ResiDual"]
    dropout_rate: float = 0.0
    ...
    def setup(self):
        ...
        outerproduct_ = (
            nn.checkpoint(OuterProduct)
            if self.global_config.remat_flag
            else OuterProduct
        )
        self.outerproduct = outerproduct_(
            global_config=self.global_config,
        ...
        self.post_outerproduct = PostNonLinear(
            global_config=self.global_config,
    ...
    def __call__(self, seq_act, pair_act, accumulated_pair_act, attention_masks):
        ...
        residual_act, accumulated_act = self.post_outerproduct(
            residual_act, outerproduct_output, pair_mask, accumulated_act
        )

        ffn_output = self.ffn(residual_act)
        residual_act, accumulated_act = self.post_ffn(
...
class FlashEvoformerStack(nn.Module):
    ...
    post_ffn_operation_list: tuple = (
        "Dropout",
    )  # ["LN", "Dropout", "ResidualLN"] ### Include "ResiDual" at the last layer.
    ...
    def setup(self):
        ...
        self.prepare_pair_act = PostNonLinear(
            global_config=self.global_config,
            o_data_dim=self.pair_act_dim,
            operation_list=("LN",),
        ...
        self.prepare_seq_act = PostNonLinear(
            global_config=self.global_config,
    ...
    def __call__(
        ...
        pair_mask_flatten = jnp.reshape(
            attention_masks[-1], flatten_shape[:-1]
        )  # (B,Q*K) # [Q,K]
        seq_mask = attention_masks[0]  # (B,Q) # [Q,]

        # (B,Q*K,c): # [Q,K,c]
        pair_act_prepared, _pair_act_residual = self.prepare_pair_act(
            pair_act_flatten,
            0.0 * pair_act_flatten,
            pair_mask_flatten,
            pair_act_residual,
        )
        # pair_act_combined = self.postln_scale*pair_act_flatten + pair_act_residual # 模仿ResidualTransformer的final layer
        pair_act_prepared = jnp.reshape(
            pair_act_prepared, pair_act_shape
        )  # (B,Q,K,c) # [Q,K,c]

        # (B,Q,c):
        seq_act_prepared, _seq_act_residual = self.prepare_seq_act(
            seq_act, 0.0 * seq_act, seq_mask, accumulated_seq_act
        )  # [Q,c]
