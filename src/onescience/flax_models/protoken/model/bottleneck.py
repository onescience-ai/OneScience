...
class BottleneckModel(nn.Module):
    ...
    def setup(self):
        ...
        self.pair_activations = nn.Dense(
            features=self.pair_channel,  # 64 -> 192
        ...
        if self.inverse_folding:
            self.module_inverse_folding = InverseFoldingHead(
                global_config=self.global_config, cfg=self.cfg.inverse_folding
            )
    ...
    def __call__(self, single_act, seq_mask, residue_index):
        ...
        pair_act = self.pair_activations(rel_pos).astype(
            self._dtype
        )  # [num_batch, Nres, Nres, 192]
        ...
        for i in range(self.single_update_transformer_stack_num):
            single_act, acc_single_act = self.single_residual_update_stack[i](
                act=single_act,  # [num_batch, Nres, 384]
