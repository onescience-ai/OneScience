...
class ConfidenceLoss:
    def __init__(self, config):
        ...
        self.bin_center = jnp.array(
            jnp.linspace(
                self.lddt_min + self.bin_width / 2,
                self.lddt_max - self.bin_width / 2,
                self.num_bins,
            ),
            jnp.float32,
        )
    ...
    def softmax_cross_entropy_loss(self, plddt, lddt, seq_mask):
        ...
        lddt_one_hot = jnp.logical_and(
            lddt >= self.bin_edge_lower[None, ...],
            lddt < self.bin_edge_upper[None, ...],
        ).astype(
            jnp.float32
        )  # (Nres, Nbins)
        ...
        if self.label_smoothing > 0.0:
            ...
            ) * loss + self.label_smoothing * smoothed_loss
