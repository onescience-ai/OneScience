...
class OrdinalXCE:
    ...
    def __call__(self, prediction_logits, target_tensor):
        ...
        # (...):
        xent_loss = softmax_cross_entropy(
    ...
    def __call__(self, logits, labels):
        ...
        if self.not_focal:
            ...
            focal_loss = -self.alpha * jnp.power(1 - positive_pt, self.gamma) * jnp.log(
                jnp.clip(positive_pt, self.epsilon, 1.0)
            ) - (1 - self.alpha) * jnp.power(1 - negative_pt, self.gamma) * jnp.log(
                jnp.clip(negative_pt, self.epsilon, 1.0)
            )
            focal_loss *= 2.0
...
class EstogramHead:
    ...
    def __call__(self, distogram_logits, dist_gt, dist_mask, cutoff):
        ...
        estogram, esto_centers, square_centers_pad = self.compute_estogram(
            distogram_logits, dmat_ref
        )
        # (Nres,Nres):
        pair_errors = jnp.sum(estogram * esto_centers, -1)

        # (Nres,Nres):
        p1 = self._integrate(distogram_logits, jnp.abs(esto_centers) < 0.5).astype(
            jnp.float32
        )
        p2 = self._integrate(distogram_logits, jnp.abs(esto_centers) < 1.0).astype(
            jnp.float32
        )
        p3 = self._integrate(distogram_logits, jnp.abs(esto_centers) < 2.0).astype(
            jnp.float32
        )
        p4 = self._integrate(distogram_logits, jnp.abs(esto_centers) < 4.0).astype(
            jnp.float32
        )

        # (Nres,Nres):
        p0 = self._integrate(distogram_logits, square_centers_pad < cutoff).astype(
            jnp.float32
        )
        # (Nres,Nres):
...
class CA_DistogramLoss:
    def __init__(self, config):
        ...
        self.contact_loss = BinaryFocalLoss(
            alpha=self.focal_alpha,
            gamma=self.focal_gamma,
            epsilon=1e-5,
            feed_in_logit=False,
            not_focal=bool(not self.if_focal_loss),
        )
    ...
    def __call__(
        ...
        # ():
        random_cutoff_mask = jax.random.uniform(
            rng_key,
            dtype=self.dtype,
            minval=self.contact_cutoff_min,
            maxval=self.contact_cutoff_max,
        )
        # (P,1,1):
        ...
        # (), (P):
        dmat_loss, opt_perm_mask = self.distogram_loss_func(
            distogram_logits_flat,
            dist_gt_perms,
            dist_mask_perms,
            perms_padding_mask,
            random_cutoff_mask_flat,
        )
        ...
        # (), ():
        lddt_loss, contact_loss = self.lddt_loss_func(
            distogram_logits, dist_gt_opt, dist_mask_opt, random_cutoff_mask
        )
    ...
    def random_cutoff_mask(self, dist_mask_perms):
        ...
        cutoff = jax.random.uniform(
            self.make_rng("uniform"),
            dist_mask_perms.shape[:1],
            dtype=self.dtype,
            minval=self.contact_cutoff_min,
            maxval=self.contact_cutoff_max,
        )  # (P,)
    ...
    def distogram_loss_func(
        self, logits, dist_gt_perms, dist_mask_perms, perms_padding_mask, cutoff_mask
    ):
        """Log loss of a distogram."""
    ...
    def lddt_loss_func(self, distogram_logits, dist_gt, dist_mask, cutoff_mask):
        ...
        plddt, pred_contact, pair_errors_ = self.lddt_loss(
            distogram_logits, dist_gt, dist_mask, cutoff=cutoff_mask
        )
