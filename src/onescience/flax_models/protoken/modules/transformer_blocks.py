...
from flax.linen.initializers import normal, ones_init, zeros_init
...
class Attention(nn.Module):
    ...
    def setup(self):
        ...
        self.linear_q = nn.Dense(
            features=self.num_head * self.dim_per_head,
        ...
        self.zero = jnp.array([0.0], self._dtype)
    ...
    def _init_parameter(self):
        ...
        #                                   [self.num_head * self.dim_per_head, self.m_data_dim], jnp.float32)
        self.linear_output_weight = self.param(
            "linear_output_weight",
            zeros_init(),
            [self.output_dim, self.num_head * self.dim_per_head],
            jnp.float32,
        )
...
class PreNonLinear(nn.Module):
    ...
    def setup(self):
        ...
        self.attention_dropout = (
            True
            if "AttDropout" in self.operation_list and self.dropout_rate > 1e-3
            else False
        )
        ...
        self.wmatmul = TransMatMul(transpose_b=True)
        self.zero = jnp.array([0.0], self._dtype)
        self.one = jnp.array([1.0], self._dtype)
    ...
    def __call__(
        ...
        if self.pair_embedding:
            ...
            # (Q,K,cz):
            pair_act_bias += masked_layer_norm(
        ...
        if self.attention_dropout:
            attention_mask_2d = self.dropout(
                attention_mask_2d
            )  # haven't set broadcast dims, maybe no need for this
    ...
    def _init_parameter(self):
        ...
        if self.pair_embedding:
            self.feat_2d_weight = self.param(
                "feat_2d_weight",
                normal(stddev=1 / np.sqrt(self.pair_act_dim)),
                (self.num_head, self.pair_act_dim),
                jnp.float32,
            )
            if self.norm_method == "layernorm":
                self.norm_p = ActFuncWrapper(
                    nn.LayerNorm(
                        epsilon=self.norm_small,
                        dtype=self._dtype,
                        param_dtype=jnp.float32,
...
class PostNonLinear(nn.Module):
    ...
    def setup(self):
        ...
        self.dropout = (
            True
            if "Dropout" in self.operation_list and self.dropout_rate > 1e-3
            else False
        )
        self.ln = True if "LN" in self.operation_list else False
    ...
    def __call__(self, act, o_data, o_mask=jnp.array([1.0]), accumulated_act=0.0):
        ...
        if self.execute_residual:
            accumulated_act += (
                o_act * self.accumulated_scale
            )  # zhenyu: * float will not change its dtype
...
class OuterProduct(nn.Module):
    ...
    def setup(self):
        ...
        self.left_projection = nn.Dense(
            features=self.outerproduct_dim,
            kernel_init=nn.initializers.lecun_normal(),
            use_bias=True,
