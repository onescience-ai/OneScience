...
class MultiRigidSidechain(nn.Module):
    ...
    def setup(self):
        ...
        self.restype_atom14_to_rigid_group = jnp.array(
            residue_constants.restype_atom14_to_rigid_group, dtype=jnp.int32
        )
        self.restype_atom14_rigid_group_positions = jnp.array(
            residue_constants.restype_atom14_rigid_group_positions, dtype=self._dtype
        )
        self.restype_atom14_mask = jnp.array(
            residue_constants.restype_atom14_mask, dtype=self._dtype
        )
        self.restype_rigid_group_default_frame = jnp.array(
            residue_constants.restype_rigid_group_default_frame, dtype=self._dtype
        )
        ...
        self.input_projection = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.input_projection_1 = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.resblock1 = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.resblock2 = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.zeros_init(),
            dtype=self._dtype,
        )
        self.resblock1_1 = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.resblock2_1 = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.zeros_init(),
            dtype=self._dtype,
        )
        self.unnormalized_angles = nn.Dense(
            features=14,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
    ...
    def __call__(self, rotation, translation, act, initial_act, aatype):
        ...
        angles = safe_l2_normalize(
            unnormalized_angles, axis=-1, epsilon=self.norm_small
        )
        ...
        all_frames_to_global = torsion_angles_to_frames(
            aatype, backb_to_global, angles, self.restype_rigid_group_default_frame
        )
...
class FoldIteration(nn.Module):
    ...
    def setup(self):
        ...
        self.attention_layer_norm = nn.LayerNorm(
            epsilon=self.norm_small, dtype=self._dtype, param_dtype=jnp.float32
        )  # [384]
        self.transition_layer_norm = nn.LayerNorm(
            epsilon=self.norm_small, dtype=self._dtype, param_dtype=jnp.float32
        )  # [384]
        ...
        # default dense setting
        self.transition = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.transition_1 = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.transition_2 = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.zeros_init(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.affine_update = nn.Dense(
            features=6,
            kernel_init=nn.initializers.zeros_init(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        ...
        attention_module_ = (
            nn.checkpoint(InvariantPointAttention)
            if self.remat_flag
            else InvariantPointAttention
        )
        self.attention_module = attention_module_(
            self.config.num_head,  # 12
            ...
            sink_attention=self.config.sink_attention,
        )  # False
        # self.mu_side_chain = BatchMultiRigidSidechain(self.config.sidechain, single_repr_dim)
        self.mu_side_chain = MultiRigidSidechain(
            self.global_config, self.config.sidechain, self.config.num_channel
        )
        # if RECOMPUTE_FLAG:
    ...
    def __call__(
        ...
        attn = self.attention_module(
            act, static_feat_2d, sequence_mask, rotation, translation
        )  # [Nres, 384]
        ...
        affine_update = (
            ...
            affine_update, dtype=self._dtype
        )
        # print('affine_update:', affine_update.shape)
        quaternion_update, rotation_update, translation_update = pre_compose(
            quaternion, rotation, translation, affine_update
        )
...
class StructureModule(nn.Module):
    ...
    def setup(self):
        ...
        # default layernorm setting
        self.single_layer_norm = nn.LayerNorm(
            epsilon=self.norm_small, dtype=self._dtype, param_dtype=jnp.float32
        )
        self.pair_layer_norm = nn.LayerNorm(
            epsilon=self.norm_small, dtype=self._dtype, param_dtype=jnp.float32
        )

        # default dense setting
        self.initial_projection = nn.Dense(
            features=self.config.num_channel,
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
    ...
    def __call__(self, single, pair, seq_mask, aatype, decoy_affine_tensor=None):
        ...
        if is_bf16:
            ...
            decoy_affine_tensor = (
                ...
                if decoy_affine_tensor is not None
                else None
            )
        ...
        # atom_pos [3, 8 , 220, 14]
        atom14_pred_positions = vecs_to_tensor(atom_pos)[
            -1, :, :, :
        ]  # [seq_len, 14, 3]
        ...
        atom14_pred_positions_split = jnp.split(
            atom14_pred_positions, (3, 4, 5), axis=-2
        )
        atom37_pred_positions = jnp.concatenate(
            [
                atom14_pred_positions_split[0],
                atom14_pred_positions_split[2],
                atom14_pred_positions_split[1],
                atom14_pred_positions_split[3],
            ],
            axis=-2,
        )
        # atom37_pred_positions = ops.Concat(axis=-2)([atom14_pred_positions[..., :3, :], atom14_pred_positions[..., 4:5, :], atom14_pred_positions[..., 3:4, :]])
        atom37_pred_positions = jnp.pad(
            atom37_pred_positions, [[0, 0], [0, 23], [0, 0]]
        )
        ...
        for iter_ in range(self.num_layer):
            if not (self.share_weights):
                (
                    ...
                    atom_pos,
                    frames,
                ) = self.fold_iteration_stack[iter_](
                    act,
                    act_2d,
                    sequence_mask,
                    quaternion,
...
class FrameInitializer(nn.Module):
    ...
    def setup(self):
        ...
        if not self.share_weights:
            ...
            self.init_iteration = FoldIteration(
                global_config=self.global_config,
                config=self.config,
                dropout_rate=self.config.dropout,
                stop_grad=self.stop_grad_ipa,
        ...
        # default dense setting
        self.initial_projection = nn.Dense(
            features=self.config.num_channel,  # 384 -> 384
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
    ...
    def __call__(self, single, pair, seq_mask, aatype):
        ...
        for iter_ in range(self.num_layer):
            if not (self.share_weights):
                ...
                (
                    ...
                    atom_pos,
                    frames,
                ) = self.init_iteration_stack[iter_](
                    act,
                    act_2d,
                    sequence_mask,
                    quaternion,
        ...
        if is_bf16:
            (affine_output_new, final_affines, single_act) = jax.tree_map(
