...
from onescience.flax_models.protoken.common.multimer_block import (
...
class FlashSingleTemplateEmbedding(nn.Module):
    ...
    def setup(self):
        ...
        self.template_mask_2d_temp_dense = nn.Dense(
            features=self.num_channels,  # 1 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.unit_vector_0 = nn.Dense(
            features=self.num_channels,  # 1 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.unit_vector_1 = nn.Dense(
            features=self.num_channels,  # 1 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        self.unit_vector_2 = nn.Dense(
            features=self.num_channels,  # 1 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        ...
        self.backbone_mask_2d_dense = nn.Dense(
            features=self.num_channels,  # 1 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        ...
        self.embedding2d = nn.Dense(
            features=self.num_channels,  # 128 -> 128
            kernel_init=nn.initializers.lecun_normal(),
            dtype=self._dtype,
            param_dtype=jnp.float32,
        )
        ...
        for _ in range(self.num_block):
            template_pair_stack_block = TemplatePairStack(
                global_config=self.global_config,
    ...
    def __call__(
        self,
        pair_activations,  # [Nres, Nres, 128]
        ...
        template_mask_2d_temp = jnp.expand_dims(
            template_pseudo_beta_mask, -1
        ) * jnp.expand_dims(template_pseudo_beta_mask, -2)

        act_tmp = self.template_mask_2d_temp_dense(
            (jnp.expand_dims(template_mask_2d_temp, -1))
        )

        backbone_mask = (
            template_all_atom_mask[:, self.n]
            * template_all_atom_mask[:, self.ca]
            * template_all_atom_mask[:, self.c]
        )

        # zhenyu: use fp32 from line122 to line132 for stability
        template_all_atom_positions_fp32 = jnp.asarray(
            template_all_atom_positions, jnp.float32
        )
        unit_vector = multimer_rigids_get_unit_vector(
            template_all_atom_positions_fp32[:, self.n],
            template_all_atom_positions_fp32[:, self.ca],
            template_all_atom_positions_fp32[:, self.c],
        )

        backbone_mask_2d = jnp.expand_dims(backbone_mask, -1) * jnp.expand_dims(
            backbone_mask, -2
        )
        ...
        for j in range(self.num_block):
            act_output = self.template_pair_stack[j](
                act_output, padding_mask_2d
            )  # [Nres, Nres, 128], [Nres, Nres] -> [Nres, Nres, 128]
