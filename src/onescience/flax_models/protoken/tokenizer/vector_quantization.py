...
def squared_euclidean_distance_fn(
    a: jnp.ndarray,
    b: jnp.ndarray,
        ...
        if train:
            e_latent_loss = jnp.sum(
                square_euclidean_distance(
                    jax.lax.stop_gradient(quantized), x, normalized=l2_norm
                )
                * mask
            ) / (jnp.sum(mask) + 1e-6)
            q_latent_loss = jnp.sum(
                square_euclidean_distance(
                    quantized, jax.lax.stop_gradient(x), normalized=l2_norm
                )
                * mask
            ) / (jnp.sum(mask) + 1e-6)
            entropy_loss = 0.0
        ...
        result_dict.update(
            {
                ...
                "code_count": jnp.bincount(
                    encoding_indices.flatten(),
                    weights=mask.flatten(),
                    length=codebook_size,
                ).astype(jnp.float32),
                "raw": x,
            }
        )
