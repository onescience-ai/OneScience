...
class LocalQwen3Bak(Runnable):
    _FACTORY_NAME = "LocalQwen3Bak"
    ...
    def invoke(
        ...
        **kwargs: Any,
        ...
        generated_ids = self.model.generate(
            **model_inputs, max_new_tokens=kwargs.get("max_new_tokens", 32768)
        )
        ...
        if content:
            match = re.match(r"```\s*json\s*(.*?)\s*```", content, re.DOTALL)
            if match:
                content = json.loads(match.group(1))
                print(f"macth {content}")
...
if __name__ == "__main__":
    ...
    from ai_core.agent.unit.tool_manager import get_tools
