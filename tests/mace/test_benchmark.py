...
def test_inference(
    ...
    with torch_tools.default_dtype(dtype):
        model = load_mace_mp_medium(
            dtype, compile_mode, device)
        batch = create_batch(size, model, device)
        log_bench_info(benchmark, dtype,
                       compile_mode, batch)

        def func():
            torch.cuda.synchronize()
            model(batch, training=compile_mode is not None,
                  compute_force=True)
...
def create_batch(size: int, model: torch.nn.Module, device: str) -> dict:
    ...
    dataset = [mace_data.AtomicData.from_config(
...
def log_bench_info(benchmark, dtype, compile_mode, batch):
    benchmark.extra_info["num_atoms"] = int(
        batch["positions"].shape[0])
    benchmark.extra_info["num_edges"] = int(
...
def process_benchmark_file(bench_file: Path) -> pd.DataFrame:
    ...
    result_df["ns/day (1 fs/step)"] = 0.086400 / \
        result_df["median"]
