...
def test_zbl_basis_initialization(zbl_basis):
    ...
    assert torch.allclose(zbl_basis.c, torch.tensor(
...
def test_agnesi_transform_forward():
    ...
    x = torch.tensor(
        [1.0, 2.0, 3.0], dtype=torch.get_default_dtype()).unsqueeze(-1)
    node_attrs = torch.tensor(
