...
def test_lmdb_dataset():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(3):
            ...
            for j in range(2):
                ...
                for _ in range(2):
                    ...
                    forces = (
                        np.random.randn(
                            *atoms.positions.shape) * 0.5
                    )  # Random forces
                    # Random stress in Voigt notation
                    stress = np.random.randn(6) * 0.2
        ...
        dataset = LMDBDataset(
            file_path=paths_str, r_max=5.0, z_table=z_table)
        ...
        item = dataset[0]
        print(item)
        assert item.positions.shape == (
            3, 3)  # 3 atoms, 3 coordinates
        ...
        assert batch.positions.shape == (
            12, 3)  # 12 atoms, 3 coordinates
        # 4 energies (one per config)
        assert batch.energy.shape[0] == 4
        assert batch.forces.shape == (
            12, 3)  # Forces for each atom
        print(batch.stress.shape)
        assert batch.stress.shape == (
            4, 3, 3)  # Stress for each config

        # Check batch has required attributes for MACE model processing
        assert hasattr(batch, "batch")  # Batch indices
        # One index per atom
        assert batch.batch.shape[0] == 12
