...
class DeNovoDesign:
    ...
    def setup_model(self):
        ...
        self.scheduler = GaussianDiffusion(
    ...
    def setup_inference_functions(self):
        ...
        self.pjit_index_from_embedding = jax.pmap(
            jax.jit(self.index_from_embedding), axis_name="i"
        )
    ...
    def denoise_step(
        self, params, x, seq_mask, t, residue_index, rng_key, clamp_x0_fn=None
    ):
        ...
        eps_prime = self.jit_apply_fn(
            ...
            seq_mask,
            t,
            tokens_rope_index=residue_index,
        )
    ...
    def run_inference(
        self, x, seq_mask, residue_index, rng_keys, n_eq_steps=50, phasing_time=250
    ):
        """Run complete inference"""
        print(
            f"Running inference with {n_eq_steps} equilibrium steps and phasing time {phasing_time}"
        )

        for ti in tqdm(range(self.num_diffusion_timesteps), desc="Diffusion steps"):
            t = self.num_diffusion_timesteps - ti
            denoise_fn = (
                self.pjit_denoise_step_clamped
                if t < phasing_time
                else self.pjit_denoise_step
            )
    ...
    def generate_proteins(
        self,
        output_dir="results/denovo_design",
        seed=8888,
        n_eq_steps=50,
        phasing_time=250,
    ):
        ...
        print(
            f"Input shapes: x={x.shape}, seq_mask={seq_mask.shape}, residue_index={residue_index.shape}"
        )
        ...
        ret = self.run_inference(
            x, seq_mask, residue_index, rng_keys, n_eq_steps, phasing_time
        )
...
def main():
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default="../ckpts/PT_DiT_params_2000000.pkl",
        help="PT-DiT checkpoint",
    )
    parser.add_argument(
        "--protoken_ckpt_path",
        type=str,
    ...
    parser.add_argument(
        "--phasing_time", type=int, default=250, help="Phasing time for clamping"
    )
    parser.add_argument(
        "--decode_structures",
        action="store_true",
        help="Decode structures after generation",
    )
