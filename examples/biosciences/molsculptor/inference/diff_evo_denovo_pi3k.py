...
def infer(args):
    ...
    def reward_function(molecule_dict, cached_dict):
        ...
        if todo_unique_smiles.size > 0:
            ...
            r_dock_3 = np.asarray(r_dock_3, np.float32) * (-1.0)
            ...
            todo_unique_scores = np.stack(
                [r_dock_1, r_dock_1_minus_2, r_dock_1_minus_3], axis=1
            )  # (N, 3)
    ...
    def constraint_function(molecule_dict, cached, config):
        ...
        logp_constraint = np.array(
            (logp >= config["logp_min"]) & (logp <= config["logp_max"]), np.int32
        )
        ## test for repeat structure constraint
        rep_constraint = find_repeats(molecule_dict["smiles"], unique_smiles)
        return np.stack(
            [rep_constraint, qed_constraint, logp_constraint, sas_constraint], axis=1
        )  # (N, 3)
    ...
    def diffusion_es(config, rng_key, init_molecules):
        ...
        x = jax.random.normal(init_key, (DEVICE_BATCH_SIZE, N_TOKENS, D_LATENT))
