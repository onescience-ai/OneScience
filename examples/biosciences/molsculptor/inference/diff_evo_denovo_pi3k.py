...
def infer(args):
    ...
    decoder_f = functools.partial(
        decoder_function, inferencer=inferencer, reverse_alphabet=reverse_alphabet
    )
    ...
    def reward_function(molecule_dict, cached_dict):
        ...
        if todo_unique_smiles.size > 0:
            ...
            todo_unique_scores = np.stack(
                [r_dock_1, r_dock_1_minus_2,
                    r_dock_1_minus_3], axis=1
            )  # (N, 3)
    ...
    def constraint_function(molecule_dict, cached, config):
        ...
        # LogP
        logp = np.asarray(LogP_reward(
            molecule_dict), np.float32)
        logp_constraint = np.array(
            (logp >= config["logp_min"]) & (
                logp <= config["logp_max"]), np.int32
        )
        # test for repeat structure constraint
        rep_constraint = find_repeats(
            molecule_dict["smiles"], unique_smiles)
        return np.stack(
            [rep_constraint, qed_constraint,
                logp_constraint, sas_constraint], axis=1
        )  # (N, 3)
    ...
    time_sched = np.array([500, 150, 100] * (TOTAL_STEP // 3 + 1), np.int32)[
        :TOTAL_STEP
    ]
    # breakpoint() ## check here
    ...
    recoder.info(
        f"=====================START INFERENCE=====================")
    output_molecules, cached = diffusion_es(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--diffusion_timesteps", type=int, default=500)
