...
from unit_sup import DEVICE_BATCH_SIZE, N_TOKENS, N_EQ_STEPS, N_REPLICATE, SAVE_PATH, jit_denoise_step, jit_noise_step, jit_noise, \
                                replicate_func,  encoder_f, recoder,  rng_key, search_config, lead_molecules,  params, args
...
def pre_infer():
    ...
    rope_index = jnp.array([np.arange(N_TOKENS),] * (DEVICE_BATCH_SIZE * N_REPLICATE), 
    ...
    cached = {
        ...
        'molecules': [{'smiles': lead_molecules['smiles'], 'graphs': lead_molecules['graphs']},], 
        'scores': [init_scores], 'sim': [init_sim_scores],
        'constraints': [init_constraints], 
        'unique_smiles': lead_molecules['smiles'][:1], 'unique_scores': init_scores[:1],}
