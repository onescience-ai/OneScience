...
def train_loop(
    dataloader, pdeloader, model, if_temporal, optimizer, device, train_args
):
    model.train()
    train_loss = 0.0
    train_data = 0.0
...
def main(args):
    ...
    train_loader, pde_loader, val_loader = get_dataloader(
        train_data, train_pde, val_data, args
    )
    # set some train args
    ...
    if not args["if_training"]:
        ...
        print("start testing...")
        print(
            f"testset batch num. is {len(val_loader)} , batchsize={args['dataloader']['batch_size']}"
        )
        # time_list=[]
        # for i in range(10):
        res, time = test_loop(
    ...
    for epoch in pbar:
        ...
        pbar.set_description(
            f"[Epoch {epoch}] train_loss: {train_loss:.5e}, data_loss: {train_data:.5e}, train_f: {train_f:.5e},train_ic:{train_ic:.5e}"
        )
