    ...
    model = nn.ModuleList(
        [
            MPNN(
                ...
                hidden_features=args["model"]["hidden_features"],
                hidden_layers=args["model"]["hidden_layer"],
                eq_variables=args["variables"],
            )
...
def val_loop(dataloader, model, loss_fn, device, graph_creator):
    ...
    for u, x, variables in dataloader:
        ...
        for step in range(
            2 * graph_creator.tw,
            graph_creator.nt - graph_creator.tw + 1,
            graph_creator.tw,
        ):
            steps = [step] * batch_size
            _, labels = graph_creator.create_data(u, steps)
            graph = graph_creator.create_next_graph(graph, output, labels, steps).to(
                device
            )
            target = torch.cat((target, graph.y), dim=-2)
            with torch.no_grad():
...
def test_loop(
    ...
    device,
    graph_creator,
    metric_names=["MSE", "RMSE", "L2RE", "MaxError"],
):
    model.eval()
    res_dict = {}
...
def main(args):
    ...
    graph_creator = GraphCreator(
        pde, neighbors=args["neighbors"], time_window=args["time_window"]
    ).to(device)
    ...
    print("Done.")
    print(
        f"Total train time: {total_time}s. Train one epoch every {total_time / (args['epochs'] - start_epoch)}s on average."
    )
