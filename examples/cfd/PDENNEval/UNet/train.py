...
def get_model(spatial_dim, args):
    ...
    if spatial_dim == 1:
        model = UNet1d(
            model_args["in_channels"] * initial_step,
    ...
    for x, y in dataloader:
        ...
        full_losses.append(
            loss_fn(
                ...
                y[..., :t_train, :].reshape(batch_size, -1),
            ).item()
        )
        # backward
        optimizer.zero_grad()
...
def main(args):
    ...
    t_train = y.shape[-2]
    unroll_step = (
        t_train - initial_step
        if t_train - initial_step < args["unroll_step"]
        else args["unroll_step"]
    )
    ...
    for epoch in range(start_epoch, args["epochs"]):
        # train loop
        train_step_loss, train_full_loss, time_spend = train_loop(
            train_loader,
            model,
            loss_fn,
            optimizer,
    ...
    print(
        f"Total train time: {total_time:.4f}s. Train one epoch every {total_time / (args['epochs'] - start_epoch):.4f}s on average."
    )
    print("Done.")
