...
class AllenCahn2DEquation(object):
    ...
    def pe_boundary(self, x_l, x_r, x_t, x_b, n=100):
        ...
        xt_bd_l, xt_bd_r = torch.cat((xl_points, y_points, t_points), dim=1), torch.cat(
            (xr_points, y_points, t_points), dim=1
        )
        xt_bd_t, xt_bd_b = torch.cat((x_points, xt_points, t_points), dim=1), torch.cat(
            (x_points, xb_points, t_points), dim=1
        )
        return (
            xt_bd_l.requires_grad_(True).to(self.device),
            xt_bd_r.requires_grad_(True).to(self.device),
            xt_bd_t.requires_grad_(True).to(self.device),
            xt_bd_b.requires_grad_(True).to(self.device),
        )
    ...
    def init_boundary(self, dataset, n=100):
        with h5py.File(dataset, "r") as f:
            ...
            idx = np.sort(idx)
            xy_init, t_init, ue_init = xy[idx, :], np.ones(
                [n, 1]) * a[2], u[idx, :]
            xt_init = torch.Tensor(
                np.concatenate([xy_init, t_init], axis=1))
...
def loss(
    ...
    int_l3 = 0.0001 * st_s * \
        torch.mean(
            torch.sum(dux * (wx * v + w * dvx), dim=1))
    ...
    loss_bd = (
        LOSS_FN(u_bd_l, u_bd_r)
        + +LOSS_FN(u_bd_t, u_bd_b)
        + LOSS_FN(dux_l, dux_r)
        + LOSS_FN(dux_t, dux_b)
    )
...
def train_pipeline():
    ...
    xt_bd_l, xt_bd_r, xt_bd_t, xt_bd_b = Eq.pe_boundary(
        a[0], b[0], b[1], a[1], n=500)
