...
def get_arguments(arg_list=None):
    ...
    parser.add_argument(
        "--i", type=int, default=20000, help="The iteration number")
    parser.add_argument(
        "--b", type=int, default=1, help="The beta number")
...
class DarcyFlowEquation(object):
    ...
    def interior(self, dataset, N=2000):
        with h5py.File(dataset, "r") as f:
            ...
            idx = np.sort(idx)
            xy_train = torch.Tensor(xy[idx, :]).requires_grad_(
                True).to(self.device)
...
def train_pipeline():
    ...
    x_test, u_real = Eq.generate_test(
        dataset, NUM_TEST_SAMPLE)
    ...
    for step in tqdm(range(NUM_ITERATION + 1)):
        ...
        loss_u, loss_v = loss(
            Eq, model_u, model_v, x_int, A, x_bd, BETA)
        start_t = time.time()
        for _ in range(2):
            loss_u, loss_v = loss(
                Eq, model_u, model_v, x_int, A, x_bd, BETA)
