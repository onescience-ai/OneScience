...
class MGNRollout:
    def __init__(self, cfg: DictConfig):
        ...
        logger.info(
            f"Using {len(self.dataset)} test samples.")
    ...
    def predict(self) -> tuple[Tensor, Tensor, Tensor]:
        ...
        for graph in self.dataloader:
            ...
            if graph.ndata["t"][0].item() == 0:
                ...
                gt_pos = []
                node_type = []
                position, vel_history, node_type = self.dataset.unpack_inputs(
                    graph)
            ...
            position = self.boundary_clamp(
                position, bounds=self.bounds)
            velocity = self.dataset.normalize_velocity(
                velocity)
            # Drop the oldest velocity and append the most recent one.
            vel_history = torch.cat(
                (vel_history[1:], velocity.unsqueeze(0)), dim=0)

            pred_pos.append(position)
            gt_pos.append(
                self.dataset.unpack_targets(graph)[0])
...
def animate(num, plotter, fig, ax1, ax2, pred, gt, node_color, bounds, frame_skip):
    num *= frame_skip
    plotter(ax1, "onescience MeshGraphNet Prediction",
            pred[num], node_color, bounds)
    plotter(ax2, "Ground Truth",
            gt[num], node_color, bounds)
...
def plot_error(mse, out_dir):
    ...
    for i, (err, color) in enumerate(zip(mse, colors)):
        ax.plot(err, marker=".", linestyle="-",
                color=color, label=f"{i}", alpha=0.6)
        ax.axhline(err.mean(), linestyle="--", color=color)
    # Global mean.
    m = np.array(mse).mean()
    ax.axhline(m, linestyle="--",
               color=colors[-1], label="All")
    ax.text(-0.1, m, f"{m:.3f}",
...
def main(cfg: DictConfig) -> None:
    ...
    for i, (pred_pos, gt_pos, node_type) in enumerate(rollout.predict()):
        ...
        ani = animation.FuncAnimation(
            fig,
            ani_func,
            frames=(rollout.num_steps -
                    rollout.num_history - 1) // rollout.frame_skip,
            interval=cfg.inference.frame_interval,
        )
        ani.save(os.path.join(
            ani_dir, f"animation_{i}.gif"))
        plt.close(fig)
