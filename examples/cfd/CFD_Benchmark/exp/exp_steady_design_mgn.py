    ...
    def vali(self):
        ...
        with torch.no_grad():
            ...
            for batch in self.test_loader:
                ...
                node_features = batch["node_features"].to(
                    self.device)
                edge_features = batch["edge_features"].to(
                    self.device)
                labels = batch["labels"].to(self.device)
                surf_mask = batch["surf_mask"].to(
                    self.device)

                # 使用新的输入格式
                out = self.model(
                    node_features, edge_features, graphs)

                # 计算损失 - 不再需要展平操作
                # 速度损失（所有节点）
                loss_velo_var = myloss(
                    out[:, :-1], labels[:, :-1]).mean(dim=0)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch in self.test_loader:
                ...
                # 分割批量数据为单个样本
                torch.split(node_features,
                            batch_num_nodes.tolist())
                out_splits = torch.split(
                    out, batch_num_nodes.tolist())
                label_splits = torch.split(
                    labels, batch_num_nodes.tolist())
                surf_mask_splits = torch.split(
                    surf_mask, batch_num_nodes.tolist())

                # 处理每个样本
                for i, (out_i, y_i, surf_i) in enumerate(
                    zip(out_splits, label_splits,
                        surf_mask_splits)
                ):
                    ...
                    if coef_norm is not None:
                        mean = torch.tensor(
                            coef_norm[2]).to(self.device)
                        std = torch.tensor(
                            coef_norm[3]).to(self.device)
                    ...
                    if not os.path.exists(data_dir_for_sample):
                        print(
                            f"Warning: Directory not found - {data_dir_for_sample}")
                        continue
