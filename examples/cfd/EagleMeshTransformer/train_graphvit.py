...
parser = argparse.ArgumentParser(
    description="Train GraphViT model on Eagle dataset")
...
def get_loss(velocity, pressure, output, state_hat, target, mask):
    ...
    rmse_velocity = torch.sqrt(
        ((velocity * mask - velocity_hat * mask)
         ** 2).mean(dim=(-1))
    )
    ...
    rmse_pressure = torch.sqrt(
        ((pressure * mask - pressure_hat * mask)
         ** 2).mean(dim=(-1))
    )
...
def validate(
    ...
    with torch.no_grad():
        ...
        for i, x in enumerate(tqdm(dataloader, desc="Validation")):
            ...
            velocity, pressure = dataloader.dataset.denormalize(
                velocity, pressure)

            costs = get_loss(
                velocity, pressure, output, state_hat, target, mask)
...
def main():
    ...
    train_sampler = DistributedSampler(
        train_dataset) if dist.world_size > 1 else None
    valid_sampler = (
    ...
    for epoch in range(args.epoch):
        ...
        for i, x in enumerate(
            ...
            velocity, pressure = train_dataset.denormalize(
                velocity, pressure)
        ...
        if dist.rank == 0:
            error = validate(
                model, valid_dataloader, epoch=epoch, device=dist.device)
            if error < memory:
                memory = error
                model_to_save = model.module if hasattr(
                    model, "module") else model
                torch.save(
                    model_to_save.state_dict(), output_ckpt)
                print("Saved!")
        else:
            error = float("inf")
