...
model = torch.load(os.path.join(
    path, f"model_{args.nb_epochs}.pth")).to(device)
...
with torch.no_grad():
    ...
    for cfd_data, geom in test_loader:
        ...
        if coef_norm is not None:  # 反归一化
            ...
            gt_press = targets[cfd_data.surf, -
                               1] * std[-1] + mean[-1]
            pred_velo = (
                out[~cfd_data.surf, :-1] *
                std[:-1] + mean[:-1]
            )  # 只需要车辆附近的速度值
            gt_velo = targets[~cfd_data.surf,
                              :-1] * std[:-1] + mean[:-1]
        ...
        np.save(
            os.path.join(
                npy_dir, f"{index}_gt.npy"), y_denorm.detach().cpu().numpy()
        )

        data_dir_for_sample = os.path.join(
            args.data_dir, f"param{args.fold_id}", vallst[index].split(
                "/")[1]
        )
        ...
        l2err_press = torch.norm(
            pred_press - gt_press) / torch.norm(gt_press)
    ...
    print(
        f"- Mean relative CD error: {(coef_error/index):.6f}")
    print("\n============ Physical Field Accuracy ================")
    l2err_press = np.mean(l2errs_press)
    l2err_velo = np.mean(l2errs_velo)
    print(
        f"- Relative L2 error (pressure): {l2err_press:.6f}")
    print(
        f"- Relative L2 error (velocity): {l2err_velo:.6f}")
    ...
    print(
        f"- Combined velocity RMSE: {np.sqrt(np.mean(np.square(rmse_velo_var))):.6f}")

    print(
        "\n============= Computational Efficiency ===============")
    print(
        f"- Mean inference time (s): {np.mean(times):.6f}")
