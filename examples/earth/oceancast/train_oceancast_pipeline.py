...
def main():
    ...
    model = build_pipeline_model(
        params, num_devices=4, chunks=params.chunks)
    # model = model.to(local_rank)
    if params["world_size"] > 1:
        # , find_unused_parameters=True
        model = DistributedDataParallel(model)

    first_stage_device = next(
        model.module[0].parameters()).device
    last_stage_device = next(
        model.module[-1].parameters()).device
    ...
    for epoch in range(params.max_epochs):
        ...
        for iteration_idx, data in enumerate(train_data_loader):
            inp = data[0].to(
                first_stage_device, dtype=torch.float)
            tar = data[1].to(
                last_stage_device, dtype=torch.float)
            # ✅ Pipe 输出为 RRef
            gen = model(inp).local_value()
        ...
        if epoch - best_loss_epoch > params.patience:
            print(
                "Loss has not decreased in 30 epochs, stopping training...")
            exit()
        if world_rank == 0:
            print(
