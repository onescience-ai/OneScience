...
class UpSample3D(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, input_resolution, output_resolution): # ( 1536*2, 1536, (8, 91, 180), (8,181,360) )
        super().__init__()
        self.linear1 = nn.Linear(in_dim, out_dim * 4, bias=False)  # ( 1536*2, 1536*4)
        self.linear2 = nn.Linear(out_dim, out_dim, bias=False)   # ( 1536, 1536)
        self.norm = nn.LayerNorm(out_dim) # 1536
        self.input_resolution = input_resolution # (8, 91, 180)
        self.output_resolution = output_resolution # (8,181,360)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # ① permute 后是非连续的；显式连续化，再进入下一次 reshape
        x = x.contiguous()
        ...
        x = self.linear2(x)
        # return x
        # ② 返回给上游/下一 stage 前，确保是连续的（P2P 更稳）
        return x.contiguous()
...
class DownSample3D(nn.Module):
    ...
    def __init__(self, in_dim, input_resolution, output_resolution): # embed_dim, (8, 181, 360), (8, 91, 180)
        super().__init__()
    ...
    def forward(self, x):
        ...
        # ① 第二次 permute 后非连续；在进入复杂 reshape 前先连续化
        x = x.contiguous()
