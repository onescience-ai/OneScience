...
def plot_samples(netcdf_file, output_dir, n_samples):
    ...
    # plotting
    truth_expanded = truth.assign_coords(
        ensemble="truth").expand_dims("ensemble")
    ens_mean = (
        ds.mean("ensemble")
    ...
    ds["ensemble"] = [str(i)
                      for i in range(ds.sizes["ensemble"])]
    merged = xarray.concat(
        [truth_expanded, ens_mean, ds], dim="ensemble")

    # plot the variables in parallel
    def plot(v):
        print(v)
        # 2 is for the ensemble and
        merged[v][: n_samples + 2,
                  :].plot(row="time", col="ensemble")
        plt.savefig(f"{output_dir}/{v}.png")

    joblib.Parallel(n_jobs=8)(
        joblib.delayed(plot)(v) for v in merged)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-samples", help="Number of samples", default=5, type=int)
    # Parse the arguments
