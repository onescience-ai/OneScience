...
for tl in timelabel:
    ...
    with h5py.File(tl, "r") as f:
        global_means += np.mean(f["fields"],
                                keepdims=True, axis=(0, 2, 3))
        global_stds += np.var(f["fields"],
                              keepdims=True, axis=(0, 2, 3))
...
for i in range(len(cfg.channels)):
    print(
        f"channel {cfg.channels[i]} means: {global_means[0, i, 0, 0]: .4f}")
for i in range(len(cfg.channels)):
    print(
        f"channel {cfg.channels[i]} stds:  {global_stds[0, i, 0, 0]: .4f}")
