...
class EvolutionLoss(nn.Module):
    ...
    def motion_regularization(self, v_x, v_y, weight):
        sobel_x = torch.tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=torch.float32).to(self.configs.device)
        sobel_y = sobel_x.T
        grad_v_x = torch.abs(F.conv2d(v_x.unsqueeze(1), sobel_x.unsqueeze(0).unsqueeze(0),
                                      padding=1, groups=1))
        grad_v_y = torch.abs(F.conv2d(v_y.unsqueeze(1), sobel_y.unsqueeze(0).unsqueeze(0),
                                      padding=1, groups=1))
    ...
    def forward(self, evo_result, evo_motion, real, v_x, v_y):
        ...
        for t in range(length):
            weight_t = torch.minimum(torch.full(real[:,t].shape, 24).to(self.configs.device), 1 + real[:,t])
