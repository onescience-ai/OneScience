...
def train_pytorch_loader(
    ...
    if dist.is_initialized():
        generator.network = DistributedDataParallel(
            generator.network,
            device_ids=[local_rank],
            output_device=[local_rank],
            find_unused_parameters=True,
        )
        discriminator.network = DistributedDataParallel(
            discriminator.network,
            device_ids=[local_rank],
            output_device=[local_rank],
            find_unused_parameters=True,
        )
        evolutionnet.network = DistributedDataParallel(
            evolutionnet.network,
            device_ids=[local_rank],
            output_device=[local_rank],
            find_unused_parameters=True,
        )
        ...
        evolutionnet.network = nn.DataParallel(
...
def TS(obs, pre, threshold=0.1):
    hits, misses, falsealarms, correctnegatives = prep_clf(
...
def test_pytorch_loader(model, test_input_handle, configs, itr):
    ...
    if configs.pretrained_model != "./data/checkpoints/mrms_model.ckpt":
        if dist.is_initialized():
            model.network = DistributedDataParallel(
                model.network,
                device_ids=[local_rank],
                output_device=[local_rank],
                find_unused_parameters=True,
    ...
    MaxPoolLoss(configs).to(configs.device)
    ...
    for batch_id, test_ims in enumerate(test_input_handle):
        ...
        def add_right_cax(ax, pad, width):
            ...
            caxpos = mtransforms.Bbox.from_extents(
                axpos.x0 - width - pad, axpos.y0, axpos.x0 - pad, axpos.y1
            )
        ...
        if batch_id <= configs.num_save_samples:
            ...
            print(
                "test_ims_plot",
                test_ims_plot.shape,
                np.max(test_ims_plot),
                np.min(test_ims_plot),
                np.mean(test_ims_plot),
            )
            print(
                "img_gen_plot",
                img_gen_plot.shape,
            ...
            for i in range(output_length):
                ...
                plot_hit_miss_fa_all_thresholds(
                    ax[2], test_ims_plot[i], img_gen_plot[i]
                )
                ax[2].set_axis_off()
