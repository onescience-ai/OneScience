...
if __name__ == "__main__":
    ...
    train_dataset = FuXiHDF5Datapipe(params=cfg, distributed=False, mode='long', num_steps=num_steps, input_steps=2)
    train_dataloader, _ = train_dataset.train_dataloader()
    print(f"Loaded train_dataloader of size {len(train_dataloader)}")
    val_dataset = FuXiHDF5Datapipe(params=cfg, distributed=False, mode='long', num_steps=num_steps, input_steps=2)
    val_dataloader, _ = val_dataset.val_dataloader()
    print(f"Loaded val_dataloader of size {len(val_dataloader)}")
    test_dataset = FuXiHDF5Datapipe(params=cfg, distributed=False, mode='long', num_steps=num_steps, input_steps=2)
    ...
    # fuxi-long is no need to infer train\val, only infer testset to calculate error.
