...
class ERA5Dataset(Dataset):
    def __init__(self, params, mode, pred_paths, gt_paths, num_steps=2, input_steps=1):
        self.params = params
        self.pred_paths = sorted(glob.glob(f"{pred_paths}/*.h5"))  # short_year.h5
        self.gt_paths = sorted(glob.glob(f"{gt_paths}/*.h5"))      # original year.h5
        self.num_steps = num_steps
        self.input_steps = input_steps
        self.mode=mode
    ...
    def _load_metadata(self):
        ...
        with h5py.File(self.pred_paths[0], 'r') as f:
            self.samples_per_year = f["fields"].shape[0] - self.num_steps - (self.input_steps - 1)
    ...
    def __getitem__(self, idx):
        if self.pred_files is None:
            self.pred_files = [h5py.File(p, 'r') for p in self.pred_paths]
            self.gt_files = [h5py.File(p, 'r') for p in self.gt_paths]
        year_idx = idx // self.samples_per_year
        step_idx = idx % self.samples_per_year
        step_idx = 0
 
        invar = self.pred_files[year_idx]["fields"][step_idx: step_idx + self.input_steps]  # [C, H, W]
        invar = torch.as_tensor(invar)

        if self.mode == 'medium':
            out_idx = step_idx + (self.params.medium_num_steps - self.params.short_num_steps) + (self.input_steps * 2 - 1)
        if self.mode == 'long':
            out_idx = step_idx + (self.params.long_num_steps - self.params.medium_num_steps) + (self.input_steps * 2 - 1)
        
        outvar = self.gt_files[year_idx]["fields"][out_idx: out_idx + self.num_steps]  # [T, C, H, W]
