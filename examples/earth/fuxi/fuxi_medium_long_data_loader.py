...
class ERA5Dataset(Dataset):
    ...
    def _load_metadata(self):
        ...
        with h5py.File(self.pred_paths[0], "r") as f:
            self.samples_per_year = (
                f["fields"].shape[0] - self.num_steps - (self.input_steps - 1)
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "medium":
            out_idx = (
                step_idx
                + (self.params.medium_num_steps - self.params.short_num_steps)
                + (self.input_steps * 2 - 1)
            )
        if self.mode == "long":
            out_idx = (
                step_idx
                + (self.params.long_num_steps - self.params.medium_num_steps)
                + (self.input_steps * 2 - 1)
            )
