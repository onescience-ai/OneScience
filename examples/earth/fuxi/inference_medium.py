...
def inference(data_loader, data_dir, step, model):
    ...
    save_path = os.path.join(data_dir, "long")
...
if __name__ == "__main__":
    ...
    num_steps = cfg.medium_num_steps - cfg.short_num_steps
    train_dataset = FuXiHDF5Datapipe(params=cfg, distributed=False, mode='medium', num_steps=num_steps, input_steps=2)
    train_dataloader, _ = train_dataset.train_dataloader()
    print(f"Loaded train_dataloader of size {len(train_dataloader)}")
    val_dataset = FuXiHDF5Datapipe(params=cfg, distributed=False, mode='medium', num_steps=num_steps, input_steps=2)
    val_dataloader, _ = val_dataset.val_dataloader()
    print(f"Loaded val_dataloader of size {len(val_dataloader)}")
    test_dataset = FuXiHDF5Datapipe(params=cfg, distributed=False, mode='medium', num_steps=num_steps, input_steps=2)
    test_dataloader = test_dataset.test_dataloader()
    print(f"Loaded test_dataloader of size {len(test_dataloader)}")
    # === 加载模型 ===
    ckpt = torch.load(f"{cfg.checkpoint_dir}/fuxi_medium.pth", map_location='cuda:0')
    fuxi_model = Fuxi(
