...
def get_error(mode, pred_path, label):
    ...
    for k in range(len(pred_path)):
        ...
        print(
            f"read {pred_path[k]}, shape: {pred.shape}, cost {time.perf_counter() - st: .2f}s"
        )
    ...
    ), f"The files are not identical in pred and truth, please check. Details: pred-{pred}; truth-{truth}"
    pred = pred.reshape(-1, pred.shape[-3],
                        pred.shape[-2], pred.shape[-1])
    label = label.reshape(-1, label.shape[-3],
                          label.shape[-2], label.shape[-1])
    label = label[-pred.shape[0]:]
    print(
        f"✅ Pred files processed.  Total {len(label_path)} files. Final shapes: pred={pred.shape}, label={label.shape}"
    )
...
if __name__ == "__main__":
    ...
    for k in range(len(label_path)):
        ...
        print(
            f"read {label_path[k]}, shape: {label.shape}, cost {time.perf_counter() - st: .2f}s"
        )
    ...
    print(
        f"Ground-truth files processed. Total {len(label_path)} files. shapes: label={label_all.shape}"
    )

    short_pred_files = sorted(
        glob.glob(f"{cfg.test_data_dir}/medium/*.h5"))
    medium_pred_files = sorted(
        glob.glob(f"{cfg.test_data_dir}/long/*.h5"))
    long_pred_files = sorted(
        glob.glob(f"{cfg.test_data_dir}/final/*.h5"))
