...
def main():
    ...
    if os.path.exists(f"{cfg.checkpoint_dir}/model_base_bak.pth"):
        if world_rank == 0:
            print("\n\n")
            print("-" * 50)
...
def save_checkpoint(model, optimizer, scheduler, best_valid_loss, best_loss_epoch, model_path):
    ...
    torch.save(state, f"{model_path}/model_base.pth")
    ### the weight file saving may interrupted due to DCU queue limit, get a backup to ensure there at least has one model 
    os.system(f"mv {model_path}/model_base.pth {model_path}/model_base_bak.pth")
