...
def download_wave(
    start_year, start_month, start_day, end_year, end_month, end_day, out_dir
):
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print(
            "Usage: python copernicusmarine_wave_mirror.py  <start_year> <start_month> <start_day> <end_year> <end_month> <end_day>"
        )
        sys.exit(1)
    ...
    print(
        f"downloading data from {start_year}/{start_month}/{start_day} to {end_year}/{end_month}/{end_day}, data will be saved at {out_dir}"
    )
    download_wave(
        start_year, start_month, start_day, end_year, end_month, end_day, out_dir
    )
