...
def show_compare(channel_indices,channel_pred):
    pth_rmse0 =  np.load('../result/rmse.npy')
    pth_rmse =  pth_rmse0[channel_pred]
    onnx_rmse = np.load('./result/rmse_onnx.npy')

    pth_acc0 = np.load('../result/acc.npy')
    pth_acc = pth_acc0[channel_pred]
    onnx_acc = np.load('./result/acc_onnx.npy')  
    
    #找到对应位置
    
    channels = [cfg_data.dataset.channels_out[i] for i in range(len(channel_pred))]
    w = 36  # 最长 channel 名宽度
...
def plot_rmse_comparison(channel_indices, filename='./result/rmse_comparison.png'):
    ...
    pth_rmse0 =  np.load('../result/rmse.npy')
    pth_rmse =  pth_rmse0[channel_indices]
    onnx_rmse = np.load('./result/rmse_onnx.npy')
...
if __name__ == "__main__":
    ...
    channels_out = cfg_data.dataset.channels_out
    ...
    channel_pred=[channels.index(v) for v in cfg_data.dataset.channels_out]
    show_compare(channel_indices,channel_pred)
    plot_rmse_comparison(channel_pred, filename='./result/rmse_comparison.png')
