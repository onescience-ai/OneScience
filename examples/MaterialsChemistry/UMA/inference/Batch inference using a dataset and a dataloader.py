...
db_path = (
    # 替换为你的数据集路径
    "../dataset/omat24/val/rattled-300-subsampled/data.aselmdb"
)

# 加载数据集
dataset = AseDBDataset(
    config=dict(
        src=db_path, a2g_args=dict(
            task_name="omat")  # 确保任务名称与OMat24数据集匹配
    )
)

# 创建DataLoader
loader = DataLoader(
    dataset, batch_size=16, collate_fn=atomicdata_list_to_batch)
...
for i, batch in enumerate(loader):
    ...
    for j in range(len(preds["energy"])):
        ...
        forces = preds["forces"][batch.batch ==
                                 j].cpu().numpy()
