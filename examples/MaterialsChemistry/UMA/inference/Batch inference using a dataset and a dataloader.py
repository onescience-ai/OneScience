...
# 数据集路径
db_path = (
    "../dataset/omat24/val/rattled-300-subsampled/data.aselmdb"  # 替换为你的数据集路径
)

# 加载数据集
dataset = AseDBDataset(
    config=dict(
        src=db_path, a2g_args=dict(task_name="omat")  # 确保任务名称与OMat24数据集匹配
    )
)

# 创建DataLoader
loader = DataLoader(dataset, batch_size=16, collate_fn=atomicdata_list_to_batch)
